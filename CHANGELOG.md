...
**Summary:** Closed the remaining **Narrative Governance & Revision Foundation MVP** gaps with minimal, contract-preserving patches across backend service logic, admin API surface, world-engine orchestration wiring, operator UI actions, and targeted cross-service tests. The release removes false-positive lifecycle success paths (especially promotion/rollback reload handling), activates rule-driven governance notifications, hardens revision permission enforcement, and updates acceptance evidence so matrix claims match executable test coverage.
...
- **Backend:** `python -m pytest tests/test_narrative_governance_service.py tests/test_narrative_governance_routes.py` (run from `backend/`) — passed.
- **World-engine:** `python -m pytest tests/test_narrative_governance_api.py` (run from `world-engine/`) — passed.
