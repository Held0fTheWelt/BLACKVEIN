...
- **AI stack — playability and self-correction:** `ai_stack/story_runtime_playability.py`, LangGraph executor retries and opening leniency, LangChain bridge correction block (`ai_stack/langgraph/langgraph_runtime_executor.py`, `ai_stack/langchain_integration/bridges.py`).
...
- **Capability invocation audit summaries (hub wave DS-025):** [`ai_stack/capabilities_invocation_summaries.py`](ai_stack/capabilities_invocation_summaries.py) — per `wos.*` handlers and `summarize_invocation_result`; [`ai_stack/capabilities.py`](ai_stack/capabilities.py) aliases `_summarize_invocation_result` (lazy import of `build_retrieval_trace` for the context-pack branch to avoid a load-time import cycle).
