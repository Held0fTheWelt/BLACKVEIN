...
- **Despaghettify autonomous machine guards:** `python -m despaghettify.tools autonomous-init` | `autonomous-advance` | `autonomous-status` | `autonomous-verify` (state under `despaghettify/state/artifacts/autonomous_loop/`, schema [`despaghettify/tools/schemas/autonomous_state.schema.json`](despaghettify/tools/schemas/autonomous_state.schema.json)); [`spaghetti-autonomous-agent-task.md`](despaghettify/spaghetti-autonomous-agent-task.md) and router skill mandate CLI checkpoints; [`despaghettify/spaghetti-setup.json`](despaghettify/spaghetti-setup.json) mirrors numeric policy for tools; **`metrics-emit`**, **`trigger-eval`**, and **`check --with-metrics`** ([`despaghettify/tools/metrics_bundle.py`](despaghettify/tools/metrics_bundle.py)); unit tests under [`despaghettify/tools/tests/`](despaghettify/tools/tests/); CI steps in [`.github/workflows/despaghettify-skills-validate.yml`](.github/workflows/despaghettify-skills-validate.yml). Run **`python despaghettify/tools/sync_despag_skills.py`** after skill edits.
- **Despaghettify autonomous macro loop:** [`despaghettify/spaghetti-autonomous-agent-task.md`](despaghettify/spaghetti-autonomous-agent-task.md) defines **check → solve (per DS) → check** until [`spaghetti-setup.md`](despaghettify/spaghetti-setup.md) conditions clear and no open **DS-*** remain, with **in-scope** hard stops and **anti-stall**; router skill [`despaghettify/superpowers/despaghettify-autonomous-agent/SKILL.md`](despaghettify/superpowers/despaghettify-autonomous-agent/SKILL.md); [`despaghettify/README.md`](despaghettify/README.md), [`despaghettify/superpowers/README.md`](despaghettify/superpowers/README.md), [`despaghettify/superpowers/despaghettify-orchestrate/SKILL.md`](despaghettify/superpowers/despaghettify-orchestrate/SKILL.md), [`despaghettify/superpowers/references/WORKFLOW.md`](despaghettify/superpowers/references/WORKFLOW.md), [`AGENTS.md`](AGENTS.md), and [`despaghettify/superpowers/references/CLI.md`](despaghettify/superpowers/references/CLI.md) updated; run **`python despaghettify/tools/sync_despag_skills.py`** after pull.
- **Despaghettify numeric policy file:** [`despaghettify/spaghetti-setup.md`](despaghettify/spaghetti-setup.md) — single editable source for **C1..C7** trigger bars, **M7** category weights, and derived **`M7_ref`**; [`spaghetti-check-task.md`](despaghettify/spaghetti-check-task.md), the implementation input list, **EMPTY** template, reset/solve/add-task docs, hub **README**, and router **skills** reference **setup** for numbers and the check task for procedure (run **`python despaghettify/tools/sync_despag_skills.py`** after skill edits).
- **Despaghettify router skills (clean / reset):** [`despaghettify/superpowers/despaghettify-clean/SKILL.md`](despaghettify/superpowers/despaghettify-clean/SKILL.md) and [`despaghettify/superpowers/despaghettify-reset/SKILL.md`](despaghettify/superpowers/despaghettify-reset/SKILL.md) mirror **check** / **solve** / **add-task** — each points at a single canonical task file (`spaghetti-clean-task.md`, `spaghetti-reset-task.md`). [`despaghettify/superpowers/README.md`](despaghettify/superpowers/README.md), [`despaghettify/superpowers/despaghettify-orchestrate/SKILL.md`](despaghettify/superpowers/despaghettify-orchestrate/SKILL.md), and [`despaghettify/superpowers/references/WORKFLOW.md`](despaghettify/superpowers/references/WORKFLOW.md) updated; run **`python despaghettify/tools/sync_despag_skills.py`** to refresh **`.cursor/skills/`**.
- **Despaghettify CLI:** [`despaghettify/tools/despaghettify.py`](despaghettify/tools/despaghettify.py) — invoke as **`python -m despaghettify.tools`** (or the script path); `check` (JSON report: AST stats via `collect_ast_stats()`, `ds005`, builtins + runtime greps), `open-ds` (open **DS-*** ids from the input list), `solve-preflight --ds DS-nnn` (exit **0** if open, **2** if missing/closed; JSON includes **`wave_sizing`** heuristics when open), **`wave-plan-validate --file`** (schema for persisted **`wave_plan.json`**). In-repo skills under [`despaghettify/superpowers/`](despaghettify/superpowers/) reference this runner as the automation entry point. Schema: [`WAVE_PLAN_SCHEMA.md`](despaghettify/superpowers/references/WAVE_PLAN_SCHEMA.md); example: [`despaghettify/tools/fixtures/despag_wave_plan_example_valid.json`](despaghettify/tools/fixtures/despag_wave_plan_example_valid.json).
- **Cursor skill discovery:** root [`.gitignore`](.gitignore) now allows **`.cursor/skills/**`** (other `.cursor/*` stays ignored). [`despaghettify/tools/sync_despag_skills.py`](despaghettify/tools/sync_despag_skills.py) copies `despaghettify/superpowers/*/SKILL.md` → `.cursor/skills/<name>/SKILL.md` (`--check` for CI drift). Added [`.cursor/skills/README.md`](.cursor/skills/README.md) and root [`AGENTS.md`](AGENTS.md) pointer.
- **Despaghettify skill path validation:** [`despaghettify/tools/validate_despag_skill_paths.py`](despaghettify/tools/validate_despag_skill_paths.py) checks links / backtick repo paths under `despaghettify/superpowers/**/*.md`; workflow [`.github/workflows/despaghettify-skills-validate.yml`](.github/workflows/despaghettify-skills-validate.yml); optional pre-commit snippet in [`despaghettify/superpowers/references/VALIDATION.md`](despaghettify/superpowers/references/VALIDATION.md).
- **Despaghettify single source of truth:** [`spaghetti-check-task.md`](despaghettify/spaghetti-check-task.md) (and sibling solve/add-task docs) declare **sole** procedure per track; **numeric** trigger policy is in [`spaghetti-setup.md`](despaghettify/spaghetti-setup.md). Hub **skills** are **routers** only (`.cursor/skills` refreshed via [`despaghettify/tools/sync_despag_skills.py`](despaghettify/tools/sync_despag_skills.py)).
- **Despaghettify / Cursor on Windows:** [`despaghettify/superpowers/references/CURSOR_ACTIVATION.md`](despaghettify/superpowers/references/CURSOR_ACTIVATION.md) documents **copy-only** sync (no symlinks / `mklink`); [`.cursor/skills/README.md`](.cursor/skills/README.md) and [`AGENTS.md`](AGENTS.md) aligned.
- **Despaghettify autonomous `backlog-implement`:** while a backlog **DS-*** row stays **open**, record each implementation slice with **`autonomous-advance --kind backlog-implement --ds … --check-json …`** (repo-relative path to hub **`check --out`** JSON); **`backlog-solve`** only after the row is closed (**exit 2** otherwise). Implementation in [`despaghettify/tools/autonomous_loop.py`](despaghettify/tools/autonomous_loop.py), [`despaghettify/tools/despaghettify.py`](despaghettify/tools/despaghettify.py), schema and tests; task doc, router skill, hub **README** tables, and [`CLI.md`](despaghettify/superpowers/references/CLI.md).
...
- **Despaghettify solve:** [`spaghetti-solve-task.md`](despaghettify/spaghetti-solve-task.md) adds **wave sizing** (bound **N**, simple-task-sized slices), **persisted wave plan** when **N > 1**, **session-limited autonomy**, and **resume after interruption** (work log + continue at **k+1**); router skill pointer updated. Follow-up: **machine `wave_plan.json`**, **`wave-plan-validate`**, **`solve-preflight` → `wave_sizing`**, **merge/rebase between waves**, and clearer **same-session** autonomy wording. CI workflow runs **`despaghettify/tools/sync_despag_skills.py --check`**, **`wave-plan-validate`** on the fixture, and **`solve-preflight --ds DS-016`** smoke.

### Changed

- **Repository language — single canonical policy:** [`docs/dev/contributing.md`](docs/dev/contributing.md) heading **Repository language** (`#repository-language`) is the **only** place the full strict-English rules live (project-wide maintainer- and operator-facing text, with explicit carve-out for in-world narrative under `content/modules/`). [`AGENTS.md`](AGENTS.md) and [`CONTRIBUTING.md`](CONTRIBUTING.md) point there and do **not** restate the policy. [`despaghettify/state/EXECUTION_GOVERNANCE.md`](despaghettify/state/EXECUTION_GOVERNANCE.md), [`despaghettify/README.md`](despaghettify/README.md), [`despaghettify/despaghettification_implementation_input.md`](despaghettify/despaghettification_implementation_input.md), [`despaghettify/state/README.md`](despaghettify/state/README.md), and Despaghettify task Markdowns (`spaghetti-*.md`, [`spaghetti-setup.md`](despaghettify/spaghetti-setup.md)) link **up** to that anchor and keep **procedure-only** language scope where needed, avoiding parallel copies that could drift.
- **Despaghettify `metrics_bundle` (heuristic v2):** [`despaghettify/tools/metrics_bundle.py`](despaghettify/tools/metrics_bundle.py) replaces **v1** flat caps with **saturating** ``100*(1-exp(-…))`` curves capped **below 100** so proxy scores never read as “totally spaghetti”. **C4** uses **>50-line** share vs **total functions**; **C6** uses **>100-line** share — no longer the same **`L₅₀`** ladder as C4. Re-run **`python -m despaghettify.tools check --with-metrics`** and copy **`metrics_bundle`** into § *Latest structure scan* after upgrading.
- **Despaghettify `metrics_bundle` — echte Anteile + deutsche Lesesätze:** [`despaghettify/tools/metrics_bundle.py`](despaghettify/tools/metrics_bundle.py) ergänzt **`literal_rates`** (z. B. Anteil der gemessenen Funktionen mit **>100** bzw. **>50** AST-Zeilen, Verschachtelung **≥6**) und **`plain_language_de`** (ein Satz pro **C1..C7**; **C1**/**C5** erklären fehlende %-Messung). **`category_scores`** bleiben die **Trigger-Skala** gegen die Balken in **`spaghetti-setup.json`**.
- **Despaghettify `metrics_bundle` — Metrik A:** [`despaghettify/tools/metrics_bundle.py`](despaghettify/tools/metrics_bundle.py) liefert **`metric_a`** (`share_weighted_m7`): **gleiche Gewichte** wie **M7**, angewandt auf die **Anteil-Vektoren** aus **`literal_rates`** (Berichtskennzahl; **kein** Trigger-Ersatz für **`m7`**).
- **Despaghettify hub CLI ergonomics:** canonical invocation **`python -m despaghettify.tools`** via [`despaghettify/tools/__main__.py`](despaghettify/tools/__main__.py) (prepends repo root to `sys.path`); optional root [`pyproject.toml`](pyproject.toml) package **`world-of-shadows-hub`** adds console scripts **`despag-check`** and **`wos-despag`** after **`pip install -e .`**. [`spaghetti_ast_scan.py`](despaghettify/tools/spaghetti_ast_scan.py) resolves **`ROOTS`** from `__file__` so **CWD** need not be the repo root. **Deprecated** thin shims restore old paths: [`tools/spaghetti_ast_scan.py`](tools/spaghetti_ast_scan.py), [`tools/ds005_runtime_import_check.py`](tools/ds005_runtime_import_check.py). Docs ([`CLI.md`](despaghettify/superpowers/references/CLI.md), [`AGENTS.md`](AGENTS.md), solve/check tasks), [`.github/workflows/despaghettify-skills-validate.yml`](.github/workflows/despaghettify-skills-validate.yml), and router skills updated; run **`python despaghettify/tools/sync_despag_skills.py`** after skill edits.
...
- **Despaghettify — DS rows:** **Information input list** **pattern** column leads with **M7 trigger category** symbols (**`C3 ·`** …, optional multi **`C2 · C3 ·`**) per [`spaghetti-check-task.md`](despaghettify/spaghetti-check-task.md) §2; **C7** added to that doc’s per-category table; live DS table and **EMPTY** template aligned.
- **Despaghettify:** new hub playbook [`spaghetti-add-task-to-meet-trigger.md`](despaghettify/spaghetti-add-task-to-meet-trigger.md) — parameter **C1..C7**, propose **DS-*** waves to lower that category, full **Recommended implementation order** re-pass (parallelism, deps, Mermaid); linked from [`despaghettify/README.md`](despaghettify/README.md).
- **Despaghettify input list:** example **`spaghetti-add-task-to-meet-trigger` (C1)** pass — **DS-015** (`**C1 ·**` runtime import-seam hardening), phased **before** DS-014; phases **4a/4b/5** and Mermaid updated in [`despaghettification_implementation_input.md`](despaghettify/despaghettification_implementation_input.md).
- **Despaghettify:** added [`despaghettify/spaghetti-clean-task.md`](despaghettify/spaghetti-clean-task.md) — removes all session files under `despaghettify/state/artifacts/workstreams/<slug>/pre|post/` and recreates empty directories per [`WORKSTREAM_INDEX.md`](despaghettify/state/WORKSTREAM_INDEX.md); [`spaghetti-reset-task.md`](despaghettify/spaghetti-reset-task.md) now starts with this clean as **Step 0**; [`despaghettify/README.md`](despaghettify/README.md) documents the split.
- **Despaghettify trigger policy — WOS-STRICT-2 weights and tighter C1 bar:** [`despaghettify/spaghetti-setup.md`](despaghettify/spaghetti-setup.md) raises the **C1** per-category bar to **2%**, documents the **WOS-STRICT-2 (B)** weight preset (higher emphasis on **C1** and **C5**), recomputes **`M7_ref`**, and clarifies **strict** gates versus **calibration**; [`despaghettify/spaghetti-setup.json`](despaghettify/spaghetti-setup.json) regenerated with **`setup-sync`**; [`despaghettify/tools/tests/test_spaghetti_setup_audit.py`](despaghettify/tools/tests/test_spaghetti_setup_audit.py) expectations updated.
- **Structure hub — reset-then-check snapshot:** [`despaghettification_implementation_input.md`](despaghettify/despaghettification_implementation_input.md) repopulated after **`spaghetti-reset-task`** (latest scan table, **DS-001..DS-005**, recommended order + Mermaid, progress log row); refreshed machine evidence under [`despaghettify/reports/reset_check_with_metrics.json`](despaghettify/reports/reset_check_with_metrics.json) and [`reset_ast_scan_capture.txt`](despaghettify/reports/reset_ast_scan_capture.txt); brief notes in [`WORKSTREAM_AI_STACK_STATE.md`](despaghettify/state/WORKSTREAM_AI_STACK_STATE.md) and [`WORKSTREAM_BACKEND_RUNTIME_AND_SERVICES_STATE.md`](despaghettify/state/WORKSTREAM_BACKEND_RUNTIME_AND_SERVICES_STATE.md).
...
- **Despaghettify hub — workstream session trees:** [`spaghetti-clean-task.md`](despaghettify/spaghetti-clean-task.md) procedure removes all files under `despaghettify/state/artifacts/workstreams/<slug>/pre|post/` for every slug in [`WORKSTREAM_INDEX.md`](despaghettify/state/WORKSTREAM_INDEX.md) and recreates empty folders; optional ephemeral cleanup (caches, local `var/` trees, scratch under `despaghettify/` per the same doc). **Local-only** governance evidence — keep **Git**, **CI**, or **PR** history if you need prior session filenames.

### Fixed

- **Spaghetti AST scan (`collect_ast_stats`):** [`despaghettify/tools/spaghetti_ast_scan.py`](despaghettify/tools/spaghetti_ast_scan.py) now unpacks all five fields returned by `metrics()` (including the magic-literal count) when building **top-12** longest and **top-6** nesting summaries — fixes `ValueError: too many values to unpack` after `metrics()` began emitting five-tuple rows.
