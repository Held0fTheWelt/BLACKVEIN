...
**Summary:** Establishes the Phase 1 operator foundation for **AI Runtime Governance** and a canonical **World-Engine Control Center** (provider/model/route CRUD, runtime readiness, play-service aggregation, docker env alignment). A follow-up **hardening pass** sharpens the same release: admin-only feature **`manage.ai_runtime_governance`** gates governance APIs and matches nav/`auth/me`; readiness returns **headline**, **severity**, and per-blocker **`suggested_action`**; the control center adds **`operator_summary`** and actionable blocker/warning copy; the manage UI favors structured operator views with collapsible raw JSON; the dashboard gains canonical entry cards.
...
- **Runtime readiness operator payload:** [`evaluate_runtime_readiness()`](backend/app/services/governance_runtime_service.py) adds `readiness_headline`, `readiness_severity`, and `suggested_action` on each blocker row for actionable remediation text.
- **World-Engine Control Center aggregation:** [`world_engine_control_center_service.py`](backend/app/services/world_engine_control_center_service.py) adds `operator_summary` (headline + sub-lines + deep-dive hint) and `suggested_action` on blockers/warnings; [`manage_world_engine_control_center.js`](administration-tool/static/manage_world_engine_control_center.js) renders summary, control list, and humanized blocker/warning lines.
- **AI Runtime Governance manage UX:** [`operational_governance.html`](administration-tool/templates/manage/operational_governance.html) and [`manage_operational_governance.js`](administration-tool/static/manage_operational_governance.js) add a readiness overview panel, provider/model/route selection metadata, richer dropdown labels, envelope unwrap helper, and `<details>` wrappers for raw JSON.
- **Navigation and dashboard discoverability:** [`base.html`](administration-tool/templates/manage/base.html) wires AI Runtime Governance and Operational Governance nav to `manage.ai_runtime_governance`; [`dashboard.html`](administration-tool/templates/manage/dashboard.html) adds cards for AI Runtime Governance and World-Engine Control Center.
- **Docs — access control and operator workflow:** [`docs/operations/OPERATIONAL_GOVERNANCE_RUNTIME.md`](docs/operations/OPERATIONAL_GOVERNANCE_RUNTIME.md) (Phase 1 entrypoints, readiness, control center, **Access control** section) and [`backend/docs/AREA_ACCESS_CONTROL.md`](backend/docs/AREA_ACCESS_CONTROL.md) (`manage.ai_runtime_governance` and world-engine capability notes).
...
- `python -m pytest backend/tests/test_operational_governance_mvp.py backend/tests/test_world_engine_control_center.py backend/tests/test_areas_api.py` (from repo root) — passed.
