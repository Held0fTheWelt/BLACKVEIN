...
- **God of Carnage legacy keyword constants and test fixtures:** [`ai_stack/scene_director_goc_legacy_keyword_constants.py`](ai_stack/scene_director_goc_legacy_keyword_constants.py); [`scene_director_goc_legacy_keyword_candidates.py`](ai_stack/scene_director_goc_legacy_keyword_candidates.py) updated to use them; [`ai_stack/tests/goc_yaml_cache_fixtures.py`](ai_stack/tests/goc_yaml_cache_fixtures.py).
...
**Summary:** Everything merged on the default branch **after** the **`v0.6.0`** release point through the **documentation and task housekeeping checkpoint** (`b031405`), the **structure-review hub** task files (reset-then-check, scan gates, empty template), and the **despaghettification input-list closure** for **DS-002 through DS-005** (packaging payloads, narrative commit path modules, AI stack report section builders, thin user PUT and validated-turn pipeline companions, plus governance pre/post artefacts and changelog capture for this patch). **Follow-up (2026-04-10):** further **despaghettification** slices **DS-022**, **DS-024**–**DS-026**, and **DS-025** — God of Carnage legacy keyword candidates, **analytics** content queries, LangGraph **dramatic review** packaging, **capability** invocation audit summaries, plus a **spaghetti AST scan** repair and optional **workstream artefact tree** reset per hub clean task.

### Added

- **God of Carnage legacy keyword scene candidates (hub wave DS-022):** [`ai_stack/scene_director_goc_legacy_keyword_candidates.py`](ai_stack/scene_director_goc_legacy_keyword_candidates.py) — `legacy_keyword_scene_candidates`; [`ai_stack/scene_director_goc.py`](ai_stack/scene_director_goc.py) imports it as `_legacy_keyword_scene_candidates` so existing tests and callers stay stable.
