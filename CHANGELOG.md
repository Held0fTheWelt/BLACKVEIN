...
- **Central feature access resolver (Phase 1.5):** [`backend/app/auth/feature_access_resolver.py`](backend/app/auth/feature_access_resolver.py) with `FEATURE_ACCESS_RULES` (minimum privilege tier per feature), `resolve_feature_access()`, optional override hook, and stable machine-readable deny reasons; [`backend/tests/test_feature_access_resolver.py`](backend/tests/test_feature_access_resolver.py) covers tiers, areas, `/auth/me` parity helpers, and rule coverage vs `FEATURE_IDS`.
- **Feature identifier `manage.ai_runtime_governance`:** registered in [`backend/app/auth/feature_registry.py`](backend/app/auth/feature_registry.py) (admin-only) so governance APIs, navigation, and `/api/v1/auth/me` stay semantically aligned.
...
- **Hardening tests:** readiness payload and moderator denial in [`backend/tests/test_operational_governance_mvp.py`](backend/tests/test_operational_governance_mvp.py); live `operator_summary` shape in [`backend/tests/test_world_engine_control_center.py`](backend/tests/test_world_engine_control_center.py); `manage.ai_runtime_governance` in [`backend/tests/test_areas_api.py`](backend/tests/test_areas_api.py); manage/dashboard assertions in [`administration-tool/tests/test_manage_operational_governance.py`](administration-tool/tests/test_manage_operational_governance.py), [`test_manage_play_service_control.py`](administration-tool/tests/test_manage_play_service_control.py), and [`test_manage_world_engine_control_center.py`](administration-tool/tests/test_manage_world_engine_control_center.py).
- **Phase 2 AI Engineer Suite surfaces and APIs:** adds backend/admin-tool support for `runtime-dashboard`, `rag-operations`, and `ai-orchestration`, including RAG status/probe/safe actions, orchestration runtime visibility, and blocker-oriented runtime aggregation across Phase 1 and Phase 2 surfaces (files under [`backend/app/services/ai_engineer_suite_service.py`](backend/app/services/ai_engineer_suite_service.py), [`backend/app/api/v1/ai_engineer_suite_routes.py`](backend/app/api/v1/ai_engineer_suite_routes.py), and `administration-tool/templates/manage/*` + `administration-tool/static/*`).
- **Phase 2.5 controlled runtime settings layer:** adds preset catalog and bounded advanced settings APIs (`/api/v1/admin/ai/presets`, `/advanced-settings`, `/effective-config`, `/settings-changes`, reset-overrides) plus canonical `/manage/runtime-settings` UI with preset application, guardrails, effective-value/source visibility, and settings-change audit summary.
- **Shared operator status semantics (final hardening):** [`backend/app/services/runtime_status_semantics.py`](backend/app/services/runtime_status_semantics.py) centralizes `STATUS_SEMANTICS` so runtime readiness, world-engine control center, and AI engineer suite responses expose the same vocabulary and operator-facing definitions.
- **Research domain governance (strategic visibility):** feature [`manage.research_governance`](backend/app/auth/feature_registry.py) (admin tier); aggregation service [`research_domain_governance_service.py`](backend/app/services/research_domain_governance_service.py) with distinct layers (`source_intake`, `extraction_tuning`, `findings_candidates`, `canonical_truth`, `mcp_workbench`), `governance_principles` (many candidates vs one promoted package per module), and admin APIs `GET /api/v1/admin/research-domain/overview` plus `GET /api/v1/admin/research-domain/layer/<layer_id>` in [`research_domain_governance_routes.py`](backend/app/api/v1/research_domain_governance_routes.py); administration-tool routes under `/manage/research/*`, [`manage_research_governance.js`](administration-tool/static/manage_research_governance.js), templates under [`administration-tool/templates/manage/research_governance/`](administration-tool/templates/manage/research_governance/overview.html), dashboard card and nav entry in [`dashboard.html`](administration-tool/templates/manage/dashboard.html) / [`base.html`](administration-tool/templates/manage/base.html); ops documentation [`docs/operations/RESEARCH_DOMAIN_GOVERNANCE.md`](docs/operations/RESEARCH_DOMAIN_GOVERNANCE.md); tests [`backend/tests/test_research_domain_governance_service.py`](backend/tests/test_research_domain_governance_service.py), [`backend/tests/test_research_domain_governance_routes.py`](backend/tests/test_research_domain_governance_routes.py), [`administration-tool/tests/test_manage_research_governance.py`](administration-tool/tests/test_manage_research_governance.py).

### Changed

- **Feature access evaluation:** [`user_can_access_feature()`](backend/app/auth/feature_registry.py) delegates to the resolver; [`feature_required_roles_legacy()`](backend/app/auth/feature_registry.py) approximates former role tuples for tooling only.
- **Governance admin routes use explicit JWT + feature checks:** [`backend/app/api/v1/operational_governance_routes.py`](backend/app/api/v1/operational_governance_routes.py) replaces blanket `@require_jwt_admin` with `@jwt_required()` + `@require_feature(FEATURE_MANAGE_AI_RUNTIME_GOVERNANCE)` (public `GET /api/v1/bootstrap/public-status` unchanged).
- **Runtime readiness operator payload:** [`evaluate_runtime_readiness()`](backend/app/services/governance_runtime_service.py) adds `readiness_headline`, `readiness_severity`, `readiness_legend`, structured inventory summaries, and `suggested_action` on each blocker row for actionable remediation text.
- **World-Engine Control Center aggregation:** [`world_engine_control_center_service.py`](backend/app/services/world_engine_control_center_service.py) adds `operator_summary` (headline + sub-lines + deep-dive hint including AI governance), `posture_at_a_glance`, `drill_down`, richer `operator_controls` entries (`requires_path_parameter`, `ui_surface`, `note`), and `suggested_action` on blockers/warnings.
- **World-Engine Control Center manage UX (operator follow-up):** [`world_engine_control_center.html`](administration-tool/templates/manage/world_engine_control_center.html) and [`manage_world_engine_control_center.js`](administration-tool/static/manage_world_engine_control_center.js) add overview status badges, connectivity glance lines, drill-down links to manage routes, blocker/warning two-line layout, clearer primary actions, and a collapsible **Technical audit** region for raw JSON payloads.
- **AI Runtime Governance manage UX (operator follow-up):** [`operational_governance.html`](administration-tool/templates/manage/operational_governance.html) and [`manage_operational_governance.js`](administration-tool/static/manage_operational_governance.js) add readiness severity row + badge, categorized blocker items with explicit “Next step”, panel hints after load, clearer action labels, and **Technical audit** `<details>` for all JSON; [`manage.css`](administration-tool/static/manage.css) adds shared operator-surface utilities (severity badges, blocker spacing).
- **Navigation and dashboard discoverability:** [`base.html`](administration-tool/templates/manage/base.html) exposes a single **AI Runtime Governance** nav entry gated by `manage.ai_runtime_governance` (duplicate Operational Governance row removed; legacy URLs unchanged); [`dashboard.html`](administration-tool/templates/manage/dashboard.html) cards for AI Runtime Governance and World-Engine Control Center use sharper operator-oriented copy.
- **Docs — access control and operator workflow:** [`docs/operations/OPERATIONAL_GOVERNANCE_RUNTIME.md`](docs/operations/OPERATIONAL_GOVERNANCE_RUNTIME.md) (Phase 1 entrypoints, readiness, control center, resolver pointer under **Access control**) and [`backend/docs/AREA_ACCESS_CONTROL.md`](backend/docs/AREA_ACCESS_CONTROL.md) (canonical feature decision path, tiers, coherence with `/auth/me` and admin `data-feature` gating).
...
- **Runtime readiness severity vocabulary:** [`evaluate_runtime_readiness()`](backend/app/services/governance_runtime_service.py) now emits `healthy` instead of `ok` for the green path and includes `status_semantics` on the readiness payload; [`manage_operational_governance.js`](administration-tool/static/manage_operational_governance.js) and [`manage.css`](administration-tool/static/manage.css) map the `healthy` badge consistently with existing OK styling.
- **World-Engine Control Center snapshot status:** [`build_world_engine_control_center_snapshot()`](backend/app/services/world_engine_control_center_service.py) adds `status.state` (`healthy`/`degraded`/`blocked`) and `status_semantics`; [`manage_world_engine_control_center.js`](administration-tool/static/manage_world_engine_control_center.js) surfaces explicit control-plane state in the overview strip.
- **AI Engineer suite semantics alignment:** [`ai_engineer_suite_service.py`](backend/app/services/ai_engineer_suite_service.py) imports shared `STATUS_SEMANTICS`, orders RAG `operational_state` so `configured_disabled` wins over empty-corpus blocked, derives governance domain status from `readiness_severity`, treats runtime-settings domain as degraded only on drift or guardrails, and limits dashboard **blockers** to truly blocked domains (RAG/orchestration) while keeping degraded detail in the warning feed.
- **Feature registry and `/auth/me` parity:** [`feature_access_resolver.py`](backend/app/auth/feature_access_resolver.py) and [`test_areas_api.py`](backend/tests/test_areas_api.py) include `manage.research_governance` alongside existing manage features.
- **Manage route registration:** [`route_registration_manage_sections.py`](administration-tool/route_registration_manage_sections.py) and [`route_registration_manage.py`](administration-tool/route_registration_manage.py) register research governance pages; [`manage.css`](administration-tool/static/manage.css) adds layout helpers for research layer cards and drill-down lists.
...
- `python -m pytest backend/tests/test_operational_governance_mvp.py backend/tests/test_world_engine_control_center.py backend/tests/test_areas_api.py backend/tests/test_feature_access_resolver.py backend/tests/test_admin_security_and_feature_registry.py` (from repo root) — passed.
...
- `python -m pytest backend/tests/test_ai_engineer_suite_routes.py -q` (from repo root) — passed.
...
- `$env:PYTHONPATH="backend"; python -m pytest backend/tests/test_operational_governance_mvp.py backend/tests/test_world_engine_control_center.py backend/tests/test_ai_engineer_suite_routes.py backend/tests/test_ai_engineer_suite_service_phase3.py` (from repo root, PowerShell) — passed.
- `python -m pytest administration-tool/tests/test_manage_operational_governance.py administration-tool/tests/test_manage_world_engine_control_center.py administration-tool/tests/test_manage_ai_engineer_suite.py` (from repo root) — passed.
- `$env:PYTHONPATH="backend"; python -m pytest backend/tests/test_research_domain_governance_service.py backend/tests/test_research_domain_governance_routes.py` (from repo root, PowerShell) — passed.
- `python -m pytest administration-tool/tests/test_manage_research_governance.py` (from repo root) — passed.
...
- **Docify hub as a documentation governance suite:** installable Python package **`docify`** under [`'fy'-suites/docify/`]('fy'-suites/docify/__init__.py) with [`docify.tools.hub_cli`]('fy'-suites/docify/tools/hub_cli.py) (subcommands **`audit`**, **`drift`**, **`open-doc`**), [`docify.tools.documentation_drift`]('fy'-suites/docify/tools/documentation_drift.py) (path-only **`git diff`** heuristics with JSON **disclaimer**), [`docify.tools.repo_paths`]('fy'-suites/docify/tools/repo_paths.py), and [`python -m docify.tools`]('fy'-suites/docify/tools/__main__.py); root [`pyproject.toml`](pyproject.toml) exposes the **`docify`** console script alongside existing hub scripts.
- **Docify task and backlog artefacts:** [`'fy'-suites/docify/documentation-check-task.md`]('fy'-suites/docify/documentation-check-task.md), [`'fy'-suites/docify/documentation-solve-task.md`]('fy'-suites/docify/documentation-solve-task.md), [`'fy'-suites/docify/documentation-reset-task.md`]('fy'-suites/docify/documentation-reset-task.md), [`'fy'-suites/docify/documentation_implementation_input.md`]('fy'-suites/docify/documentation_implementation_input.md) (**DOC-*** backlog), [`'fy'-suites/docify/templates/documentation_implementation_input.EMPTY.md`]('fy'-suites/docify/templates/documentation_implementation_input.EMPTY.md), [`'fy'-suites/docify/state/README.md`]('fy'-suites/docify/state/README.md), and [`'fy'-suites/docify/reports/.gitkeep`]('fy'-suites/docify/reports/.gitkeep) for evidence-style JSON outputs.
- **Docify documentation quality standard:** [`'fy'-suites/docify/DOCUMENTATION_QUALITY_STANDARD.md`]('fy'-suites/docify/DOCUMENTATION_QUALITY_STANDARD.md) (PEP 8 / PEP 257 layering, comment vs docstring rules, drift honesty, audit nuance for private **`ast.NodeVisitor`** **`visit_*`** hooks).
- **Docify unit tests:** [`'fy'-suites/docify/tools/tests/`]('fy'-suites/docify/tools/tests/__init__.py) (drift classification, audit behaviour, hub **`open-doc`** / **`parse_open_doc_ids`** and **`--input`** isolation).
- **Contractify hub (repository contract governance):** installable Python package **`contractify`** under [`'fy'-suites/contractify/`]('fy'-suites/contractify/__init__.py) with [`contractify.tools.hub_cli`]('fy'-suites/contractify/tools/hub_cli.py) (subcommands **`discover`**, **`audit`**, **`self-check`**), [`contractify.tools.discovery`]('fy'-suites/contractify/tools/discovery.py) (explicit A–E heuristics and `discovery_reason` per candidate), [`contractify.tools.drift_analysis`]('fy'-suites/contractify/tools/drift_analysis.py) (deterministic OpenAPI SHA vs [`postman/postmanify-manifest.json`](postman/postmanify-manifest.json), heuristic projection back-reference hints, Docify scan-root handoff, Despaghettify setup JSON presence), [`contractify.tools.conflicts`]('fy'-suites/contractify/tools/conflicts.py) (duplicate normative-index targets, bounded ADR vocabulary overlap, projection OpenAPI fingerprint mismatches, deprecated ADR header without supersession cues), [`contractify.tools.versioning`]('fy'-suites/contractify/tools/versioning.py) (OpenAPI `info.version` + explicit ADR `Status:` parsing), [`contractify.tools.relations`]('fy'-suites/contractify/tools/relations.py) (bounded **`extend_relations`** pass: normative↔OpenAPI **`references`**, index **`indexes`**, **`implements`** to **`OBS:backend/`** when present, operational runbook **`operationalizes`**), [`contractify.tools.audit_pipeline`]('fy'-suites/contractify/tools/audit_pipeline.py), [`contractify.tools.repo_paths`]('fy'-suites/contractify/tools/repo_paths.py) (optional **`CONTRACTIFY_REPO_ROOT`** override), and [`python -m contractify.tools`]('fy'-suites/contractify/tools/__main__.py); root [`pyproject.toml`](pyproject.toml) exposes the **`contractify`** console script.
- **Contractify task artefacts, scope, and backlog:** [`'fy'-suites/contractify/contract-audit-task.md`]('fy'-suites/contractify/contract-audit-task.md), [`'fy'-suites/contractify/contract-solve-task.md`]('fy'-suites/contractify/contract-solve-task.md), [`'fy'-suites/contractify/contract-reset-task.md`]('fy'-suites/contractify/contract-reset-task.md), [`'fy'-suites/contractify/CONTRACT_GOVERNANCE_SCOPE.md`]('fy'-suites/contractify/CONTRACT_GOVERNANCE_SCOPE.md) (automation thresholds and projection rule), [`'fy'-suites/contractify/state/PREWORK_REPOSITORY_CONTRACT_REALITY.md`]('fy'-suites/contractify/state/PREWORK_REPOSITORY_CONTRACT_REALITY.md), [`'fy'-suites/contractify/contract_governance_input.md`]('fy'-suites/contractify/contract_governance_input.md) (**CG-*** backlog), [`'fy'-suites/contractify/templates/contract_governance_input.EMPTY.md`]('fy'-suites/contractify/templates/contract_governance_input.EMPTY.md), and hub [`README.md`]('fy'-suites/contractify/README.md).
- **Contractify Cursor skills and sync:** [`'fy'-suites/contractify/superpowers/contractify-orchestrate/SKILL.md`]('fy'-suites/contractify/superpowers/contractify-orchestrate/SKILL.md), [`'fy'-suites/contractify/superpowers/contractify-check/SKILL.md`]('fy'-suites/contractify/superpowers/contractify-check/SKILL.md), [`'fy'-suites/contractify/tools/sync_contractify_skills.py`]('fy'-suites/contractify/tools/sync_contractify_skills.py), [`'fy'-suites/contractify/tools/validate_contractify_skill_paths.py`]('fy'-suites/contractify/tools/validate_contractify_skill_paths.py), and committed mirrors under **`.cursor/skills/contractify-*/`** after sync.
- **Contractify unit tests and fixtures:** [`'fy'-suites/contractify/tools/tests/conftest.py`]('fy'-suites/contractify/tools/tests/conftest.py) (autouse synthetic mini-repo + **`repo_root`** monkeypatch so **ZIP**/partial-tree **pytest** works), [`test_conflicts.py`]('fy'-suites/contractify/tools/tests/test_conflicts.py), [`test_versioning.py`]('fy'-suites/contractify/tools/tests/test_versioning.py), [`test_relations.py`]('fy'-suites/contractify/tools/tests/test_relations.py), [`test_example_artifacts.py`]('fy'-suites/contractify/tools/tests/test_example_artifacts.py), plus discovery/drift/hub/model tests under [`'fy'-suites/contractify/tools/tests/`]('fy'-suites/contractify/tools/tests/__init__.py).
- **Contractify committed examples:** [`'fy'-suites/contractify/examples/README.md`]('fy'-suites/contractify/examples/README.md), [`contract_discovery.sample.json`]('fy'-suites/contractify/examples/contract_discovery.sample.json), [`contract_audit.sample.json`]('fy'-suites/contractify/examples/contract_audit.sample.json) (shape samples including populated **`conflicts[]`** with **`classification`** metadata).
- **Contractify completion state note:** [`'fy'-suites/contractify/state/COMPLETION_PASS_STATE.md`]('fy'-suites/contractify/state/COMPLETION_PASS_STATE.md) (verified strengths, closed gaps, deferred follow-ups).
- **Contractify finalization state note:** [`'fy'-suites/contractify/state/FINALIZATION_PASS_2026-04-13.md`]('fy'-suites/contractify/state/FINALIZATION_PASS_2026-04-13.md) (baseline, executed slices, evidence, intentional limits).
- **Contractify last-mile closure note:** [`'fy'-suites/contractify/state/LAST_MILE_CLOSURE_2026-04-13.md`]('fy'-suites/contractify/state/LAST_MILE_CLOSURE_2026-04-13.md) (evidence fixtures, projection↔retired lifecycle signal, doc alignment).
- **Contractify committed report fixtures:** [`'fy'-suites/contractify/reports/committed/audit.hermetic-fixture.json`]('fy'-suites/contractify/reports/committed/audit.hermetic-fixture.json) and [`discover.hermetic-fixture.json`]('fy'-suites/contractify/reports/committed/discover.hermetic-fixture.json) (full discover/audit JSON from the shared minimal repo; stable `generated_at` and neutral `repo_root` label); [`'fy'-suites/contractify/tools/freeze_committed_reports.py`]('fy'-suites/contractify/tools/freeze_committed_reports.py); [`'fy'-suites/contractify/tools/minimal_repo.py`]('fy'-suites/contractify/tools/minimal_repo.py) (fixture builder for **pytest** + freeze).
- **Operational runtime governance (MVP foundation):** Alembic [`043_operational_settings_runtime_governance_foundation`](backend/migrations/versions/043_operational_settings_runtime_governance_foundation.py); SQLAlchemy models [`governance_core`](backend/app/models/governance_core.py) / [`governance_enums`](backend/app/models/governance_enums.py); shared contracts under [`backend/app/governance/`](backend/app/governance/__init__.py); [`governance_runtime_service`](backend/app/services/governance_runtime_service.py) and [`governance_secret_crypto_service`](backend/app/services/governance_secret_crypto_service.py); API blueprint [`operational_governance_routes`](backend/app/api/v1/operational_governance_routes.py); app factory hook **`ensure_governance_baseline()`** in [`factory_app.py`](backend/app/factory_app.py); world-engine [`runtime_config_client`](world-engine/app/runtime/runtime_config_client.py) plus config/main/manager wiring; administration-tool **Operational governance** page ([`operational_governance.html`](administration-tool/templates/manage/operational_governance.html), [`manage_operational_governance.js`](administration-tool/static/manage_operational_governance.js), route registration); operator notes in [`docs/operations/OPERATIONAL_GOVERNANCE_RUNTIME.md`](docs/operations/OPERATIONAL_GOVERNANCE_RUNTIME.md); [`docker-up.py`](docker-up.py) bootstrap alignment; backend tests [`test_operational_governance_mvp.py`](backend/tests/test_operational_governance_mvp.py) and administration-tool tests [`test_manage_operational_governance.py`](administration-tool/tests/test_manage_operational_governance.py); OpenAPI refresh via [`generate_openapi_spec.py`](backend/scripts/generate_openapi_spec.py).
- **Backend promotion flow (closure-critical):** [`backend/app/services/narrative_governance_service.py`](backend/app/services/narrative_governance_service.py) now includes `promote_preview_to_active()` with readiness gates, unresolved-conflict blocking, immutable version materialization from preview artifacts, append-only history entry emission, explicit `promotion_completed` event emission, and mandatory world-engine active reload orchestration.
- **World-engine governance orchestration wrappers in backend play client:** [`backend/app/services/game_service.py`](backend/app/services/game_service.py) adds typed wrappers for internal narrative endpoints: active reload, preview load/unload, preview session start/end, runtime state, validator config, and runtime health.
- **Admin governance routes for runtime orchestration and evaluation completion:** [`backend/app/api/v1/narrative_governance_routes.py`](backend/app/api/v1/narrative_governance_routes.py) adds:
  - `POST /api/v1/admin/narrative/packages/<module_id>/promote-preview`
  - `POST /api/v1/admin/narrative/runtime/health/sync`
  - `GET /api/v1/admin/narrative/runtime/diagnostics`
  - preview runtime control routes (load/unload/start-session/end-session)
  - `POST /api/v1/admin/narrative/evaluations/<run_id>/complete`
- **Runtime ingest bridge helper:** `sync_runtime_health_from_world_engine()` in [`backend/app/services/narrative_governance_service.py`](backend/app/services/narrative_governance_service.py) ingests corrective-retry and safe-fallback events from world-engine runtime health into backend governance visibility.
- **Admin-tool actionable governance controls:** [`administration-tool/static/narrative_governance.js`](administration-tool/static/narrative_governance.js) adds operator actions for build preview, promote preview, emergency rollback, conflict resolution, notification acknowledge/rule upsert, runtime sync, and evaluation run/complete workflows (with corresponding action-row styling in [`administration-tool/static/narrative_governance.css`](administration-tool/static/narrative_governance.css)).
- **New closure tests:**
  - Backend service closures in [`backend/tests/test_narrative_governance_service.py`](backend/tests/test_narrative_governance_service.py)
  - Backend route closures in [`backend/tests/test_narrative_governance_routes.py`](backend/tests/test_narrative_governance_routes.py)
  - World-engine internal governance endpoint coverage in [`world-engine/tests/test_narrative_governance_api.py`](world-engine/tests/test_narrative_governance_api.py)
- **fy_platform (Wave 1 portable governance core):** installable package under [`'fy'-suites/fy_platform/`]('fy'-suites/fy_platform/__init__.py) with [`fy_platform.core.project_resolver`]('fy'-suites/fy_platform/core/project_resolver.py) (ancestor walk with optional env override and **`fy-manifest.yaml`** / **`fy-manifest.yml`** detection), [`fy_platform.core.manifest`]('fy'-suites/fy_platform/core/manifest.py), [`fy_platform.core.models`]('fy'-suites/fy_platform/core/models.py), [`fy_platform.core.deprecation`]('fy'-suites/fy_platform/core/deprecation.py), and [`fy_platform.core.artifact_envelope`]('fy'-suites/fy_platform/core/artifact_envelope.py) (**`envelopeVersion`**, **`suiteVersion`**, **`compatMode`**, suite-neutral **`findings`** / **`evidence`** / **`stats`**, canonical **`sort_keys`** JSON dump).
- **fy_platform CLI and templates:** [`fy_platform.tools.cli`]('fy'-suites/fy_platform/tools/cli.py) / [`python -m fy_platform.tools`]('fy'-suites/fy_platform/tools/__main__.py) — subcommands **`bootstrap`** and **`validate-manifest`** with **`--project-root`** and **`FY_PLATFORM_PROJECT_ROOT`**; [`templates/fy-manifest.template.yaml`]('fy'-suites/fy_platform/templates/fy-manifest.template.yaml'); root [`pyproject.toml`](pyproject.toml) exposes **`fy-platform`** alongside existing hub scripts.
- **Wave 1 compatibility and deprecation artefacts:** [`compatibility_matrix.wave1_baseline.json`]('fy'-suites/fy_platform/compatibility_matrix.wave1_baseline.json) (per-suite CLI surfaces, exit codes, default paths, JSON key surfaces, **`operational_surfaces`** command flags and default filenames, known consuming workflows); [`deprecation_matrix.wave1.json`]('fy'-suites/fy_platform/deprecation_matrix.wave1.json) (policy visibility plus active **`DOCIFY-`**, **`POSTMANIFY-`**, **`CONTRACTIFY-`** soft-deprecation entries with **`removal_target`**).
- **Synthetic portability fixtures:** [`'fy'-suites/fy_platform/fixtures/backend_service/`]('fy'-suites/fy_platform/fixtures/backend_service/fy-manifest.yaml), **`cli_api_hybrid`**, and **`library_only`** minimal trees with manifests for regression tests.
- **Wave 1 closure documentation:** [`WAVE1_PREWORK_STANDCHECK_2026-04-13.md`]('fy'-suites/fy_platform/state/WAVE1_PREWORK_STANDCHECK_2026-04-13.md), [`WAVE1_POSTWORK_COMPLETION_2026-04-13.md`]('fy'-suites/fy_platform/state/WAVE1_POSTWORK_COMPLETION_2026-04-13.md), and [`WAVE1_CLOSURE_REPORT.md`]('fy'-suites/fy_platform/state/WAVE1_CLOSURE_REPORT.md) (stand-check, exit criteria, deferred Wave 2 items).
- **fy_platform unit tests:** [`'fy'-suites/fy_platform/tests/`]('fy'-suites/fy_platform/tests/test_manifest.py) (manifest load, compatibility matrix shape, deprecation matrix entries, project resolver, synthetic portability, artifact envelope golden file, CLI bootstrap).

### Changed

- **Preview build flow corrected (no synthetic-ID artifact mismatch):** `build_preview()` now resolves real preview artifacts via `_resolve_preview_artifact()` and optional caller-supplied `preview_id`, rather than generating a new `preview_id` and immediately expecting pre-existing files under that new id.
- **Rollback flow now enforces real runtime reload outcome:** `rollback_to_version()` no longer implies success solely from DB updates; world-engine reload is attempted as a required step and refusal is surfaced explicitly.
- **Revision transition permissions are now enforced:** `transition_revision()` now applies a role-aware transition guard (`transition_role_not_allowed`) instead of recording `actor_role` without hard enforcement.
- **Conflict resolution now has explicit strategy validation and side effects:** `resolve_conflict()` now rejects invalid strategies, enforces winner requirements where needed, and performs explicit archival side effects for loser/batch strategies.
- **Notification infrastructure switched from passive rows to event-rule execution:** `emit_narrative_event()` now evaluates enabled `NarrativeNotificationRule` entries (`event_type` + condition matching) and materializes channel/recipient-aware feed payloads.
- **Runtime fallback threshold governance is now explicit:** `ingest_runtime_health_event()` now reads runtime fallback threshold configuration and emits `fallback_threshold_exceeded` when thresholds are crossed.
- **Evaluation lifecycle closure:** `record_evaluation_run()` and new `complete_evaluation_run()` update preview evaluation/readiness state so promotion gates reflect persisted evaluation outcomes.
- **Error mapping in admin routes hardened:** governance route handlers now map critical closure codes to explicit statuses (including `403` for role-transition violations and `503` for world-engine reload refusal cases) instead of collapsing to generic outcomes.
- **Docify default AST audit scope:** [`'fy'-suites/docify/tools/python_documentation_audit.py`]('fy'-suites/docify/tools/python_documentation_audit.py) now includes **`'fy'-suites/docify`** and **`'fy'-suites/contractify`** in the default **`--root`** set so routine audits self-cover both hubs; [`AGENTS.md`](AGENTS.md), [`'fy'-suites/docify/README.md`]('fy'-suites/docify/README.md), and [`'fy'-suites/Readme.md`]('fy'-suites/Readme.md) describe the updated behaviour (this supersedes the **`[0.6.3]`** note that excluded the Docify tree from default scans).
- **Agent and contributor discovery:** [`AGENTS.md`](AGENTS.md), [`CONTRIBUTING.md`](CONTRIBUTING.md), and [`docs/dev/contributing.md`](docs/dev/contributing.md) document the Contractify hub, skill sync, and path validation commands.
- **Git hygiene:** root [`.gitignore`](.gitignore) ignores **`**/contractify/reports/*.json`** so machine-generated contract audit exports stay local unless explicitly renamed for evidence bundles.
- **Docify audit behaviour:** missing-docstring reporting skips **`visit_*`** methods on **private** (**`_`-prefixed**) **`ast.NodeVisitor`** subclasses to avoid low-signal backlog noise; visitor **`__init__`** / module entrypoints and **`main()`** gain short docstrings where appropriate; [`'fy'-suites/docify/tools/sync_docify_skills.py`]('fy'-suites/docify/tools/sync_docify_skills.py) and [`'fy'-suites/docify/tools/validate_docify_skill_paths.py`]('fy'-suites/docify/tools/validate_docify_skill_paths.py) document **`sync()`** / **`main()`** at their CLI boundaries.
- **Cursor Docify orchestration skill:** [`'fy'-suites/docify/superpowers/docify-orchestrate/SKILL.md`]('fy'-suites/docify/superpowers/docify-orchestrate/SKILL.md) routes to the new check/solve/reset tasks and the quality standard (committed copy under **`.cursor/skills/docify-orchestrate/`** after sync).
- **CI — Docify workflow:** [`.github/workflows/docify-skills-validate.yml`](.github/workflows/docify-skills-validate.yml) watches the whole **`'fy'-suites/docify/**`** tree and **`pyproject.toml`**, runs **`pip install -e .`**, installs **pytest**, and executes **`python -m pytest "'fy'-suites/docify/tools/tests" -q`** after skill path validation and sync drift check.
- **Docify `open-doc` CLI and hub tests:** [`'fy'-suites/docify/tools/hub_cli.py`]('fy'-suites/docify/tools/hub_cli.py) adds **`parse_open_doc_ids()`** (markdown-only parsing of open **DOC-*** table rows) and **`docify open-doc --input <path>`** so backlog IDs can be listed without resolving the monorepo via **`repo_paths.repo_root()`** (archives, CI checkouts, partial trees); [`'fy'-suites/docify/README.md`]('fy'-suites/docify/README.md) documents **`--input`**; [`'fy'-suites/docify/tools/tests/test_hub_cli.py`]('fy'-suites/docify/tools/tests/test_hub_cli.py) exercises the parser, explicit **`--input`**, and the default hub path using a **`monkeypatch`**ed fake repo layout so **pytest** does not depend on a real **world-of-shadows-hub** checkout root alone.
- **Operational governance wiring:** [`backend/app/config.py`](backend/app/config.py) gains settings consumed by runtime governance and play-service alignment; [`backend/app/api/v1/__init__.py`](backend/app/api/v1/__init__.py) registers operational governance routes; [`backend/app/runtime/turn_dispatcher.py`](backend/app/runtime/turn_dispatcher.py) aligns with governed runtime behaviour where applicable.
- **Contractify `discover` parity:** [`contractify.tools.hub_cli`]('fy'-suites/contractify/tools/hub_cli.py) now runs **`extend_relations()`** on the discovery payload so CLI **`relations`** match the bounded graph edges included in **`audit`**; hub README documents conflict classes, versioning behaviour, and completion state artefacts.
- **Contractify finalization (bounded completion pass):** [`contractify.tools.models`]('fy'-suites/contractify/tools/models.py) conflict rows gain **`severity`**, **`kind`**, and candidate buckets; [`contractify.tools.conflicts`]('fy'-suites/contractify/tools/conflicts.py) adds Active/Binding index row vs retired ADR detection, orphan **`source_contract_id`** checks, and structured severities; [`contractify.tools.versioning`]('fy'-suites/contractify/tools/versioning.py) parses **`Supersedes:`** for relation wiring; [`contractify.tools.relations`]('fy'-suites/contractify/tools/relations.py) emits **`supersedes`**, **`validates`**, **`conflicts_with`**, and fy handoff edges; [`contractify.tools.discovery`]('fy'-suites/contractify/tools/discovery.py) discovers **postmanify** / **docify** task anchors and only attaches OpenAPI **`implemented_by`** when **`backend/`** exists; [`contractify.tools.drift_analysis`]('fy'-suites/contractify/tools/drift_analysis.py) adds missing **`implemented_by`** path drift and postmanify task vs manifest **`openapi_path`** alignment; [`contractify.tools.audit_pipeline`]('fy'-suites/contractify/tools/audit_pipeline.py) orders drift → conflicts → conflict-aware **`extend_relations`** and prefixes **`actionable_units`** with **`[conflict:<severity>|…]`**; [`contractify.tools.hub_cli`]('fy'-suites/contractify/tools/hub_cli.py) **`discover`** uses the same conflict-aware relation pass as **`audit`**; docs ([`'fy'-suites/contractify/README.md`]('fy'-suites/contractify/README.md), [`CONTRACT_GOVERNANCE_SCOPE.md`]('fy'-suites/contractify/CONTRACT_GOVERNANCE_SCOPE.md), [`contract-audit-task.md`]('fy'-suites/contractify/contract-audit-task.md), [`examples/`]('fy'-suites/contractify/examples/)) aligned to behaviour.
- **Contractify last-mile:** [`build_discover_payload()`]('fy'-suites/contractify/tools/audit_pipeline.py) centralises discover JSON; [`detect_all_conflicts(..., contracts=…)`]('fy'-suites/contractify/tools/conflicts.py) adds **projection pins retired contract** (`lifecycle_projection_vs_retired_anchor`); [`run_audit(..., frozen_generated_at=…)`]('fy'-suites/contractify/tools/audit_pipeline.py) for reproducible fixture regeneration; [`reports/README.md`]('fy'-suites/contractify/reports/README.md) explains **`reports/committed/`** vs gitignored **`reports/*.json`**; hermetic **`conftest`** imports shared [`minimal_repo.py`]('fy'-suites/contractify/tools/minimal_repo.py).
- **Cross-suite project root and manifest alignment (Wave 1):** [`docify.tools.repo_paths`]('fy'-suites/docify/tools/repo_paths.py), [`postmanify.tools.repo_paths`]('fy'-suites/postmanify/tools/repo_paths.py), and [`contractify.tools.repo_paths`]('fy'-suites/contractify/tools/repo_paths.py) resolve the project root via **`fy_platform.core.project_resolver.resolve_project_root`** without requiring **`world-of-shadows-hub`** marker text in the primary path; **Contractify** keeps optional **`CONTRACTIFY_REPO_ROOT`**. Docify/Contractify skill sync and path validators use the same resolver.
- **Docify audit:** [`python_documentation_audit.py`]('fy'-suites/docify/tools/python_documentation_audit.py) honours manifest-driven scan roots, supports **`--envelope-out`**, emits soft-deprecation warnings and markdown sidecar when no manifest is present, and maps audit payload into the shared envelope base fields.
- **Postmanify:** [`cli.py`]('fy'-suites/postmanify/tools/cli.py) reads default OpenAPI, master collection path, and suites directory from **`fy-manifest.yaml`** when present; supports **`--envelope-out`**; legacy default collection naming emits **`DEPRECATION`** to stderr, envelope **`deprecations`**, and **`postman/postmanify.deprecations.md`** when applicable.
- **Contractify hub CLI:** [`hub_cli.py`]('fy'-suites/contractify/tools/hub_cli.py) supports **`--envelope-out`** on **`discover`** / **`audit`** / **`self-check`**, projects drift and conflict summaries into suite-neutral envelope **`findings`** / **`evidence`**, and writes a **`.deprecations.md`** sidecar next to **`--out`** JSON when legacy manifest-less mode applies.
- **Despaghettify portability:** [`repo_paths.py`]('fy'-suites/despaghettify/tools/repo_paths.py) adds **`DESPAG_REPO_ROOT`**, optional manifest **`hub_path`**, and local-hub fallback; [`hub_cli.py`]('fy'-suites/despaghettify/tools/hub_cli.py) avoids import-time hard failure, resolves runtime directory from manifest when set, runs **DS-005** only when that runtime tree exists (synthetic projects skip the gate without failing **`check`**), and accepts argv in **`main()`**; [`autonomous_loop.py`]('fy'-suites/despaghettify/tools/autonomous_loop.py), [`wave_plan_emit.py`]('fy'-suites/despaghettify/tools/wave_plan_emit.py), [`spaghetti_ast_scan.py`]('fy'-suites/despaghettify/tools/spaghetti_ast_scan.py), and [`ds005_runtime_import_check.py`]('fy'-suites/despaghettify/tools/ds005_runtime_import_check.py) tolerate unresolved roots at import time where safe; [`spaghetti_ast_scan.py`]('fy'-suites/despaghettify/tools/spaghetti_ast_scan.py) reads **`scan_roots`** from the manifest when configured.
- **Suite catalog:** [`'fy'-suites/Readme.md`]('fy'-suites/Readme.md) documents **`fy_platform`** and compatibility-matrix maintenance expectations.

### Fixed

- **False-positive lifecycle success paths removed:** promotion/rollback no longer claim runtime success when only backend state mutated.
- **Package history semantics corrected for existing modules without events:** `package_history()` now returns an empty event list when a package row exists but history is empty, while still returning `module_not_found` for absent modules.
- **Acceptance evidence no longer overstates test coverage:** [`docs/MVPs/MVP_Narrative_Governance_And_Revision_Foundation/acceptance_coverage_matrix.md`](docs/MVPs/MVP_Narrative_Governance_And_Revision_Foundation/acceptance_coverage_matrix.md) now references concrete closure tests and clarifies where verification is manual smoke vs executable API/service tests.
- **Alembic 043 — idempotent governance DDL on SQLite:** [`043_operational_settings_runtime_governance_foundation`](backend/migrations/versions/043_operational_settings_runtime_governance_foundation.py) creates each governance table (and its indexes) only when that table name is absent, matching the defensive pattern used for **`refresh_tokens`** in migration **039**, so **`flask db upgrade`** in **Docker** no longer fails with **“table … already exists”** when volumes were touched by **`db.create_all()`** or a partial upgrade while **`alembic_version`** still lagged **042**.
- **Alembic 043 — bootstrap preset bulk insert timestamps:** default preset rows now set **timezone-aware** **`created_at`** and **`updated_at`**, satisfying **`NOT NULL`** on **`bootstrap_presets`** when seeding an empty table (including the **`_seed_bootstrap_presets_if_empty()`** path).

### Removed

- **Legacy duplicate Despaghettify hub at repository root:** the stray **`despaghettify/`** tree (parallel to **`'fy'-suites/despaghettify/`**) is deleted. Path resolution in [`'fy'-suites/despaghettify/tools/repo_paths.py`]('fy'-suites/despaghettify/tools/repo_paths.py) and the deprecated shims [`tools/spaghetti_ast_scan.py`](tools/spaghetti_ast_scan.py) / [`tools/ds005_runtime_import_check.py`](tools/ds005_runtime_import_check.py) now assume the hub only under **`'fy'-suites/`**. Docs index and developer README links point at the nested paths; historical changelog anchors were retargeted to **`'fy'-suites/despaghettify/…`**.

### Tests (integrity verification)

- **Backend — narrative governance:** `python -m pytest tests/test_narrative_governance_service.py tests/test_narrative_governance_routes.py` (from **`backend/`**) — passed.
- **World-engine — narrative governance:** `python -m pytest tests/test_narrative_governance_api.py` (from **`world-engine/`**) — passed.
- **Backend — operational governance MVP:** `python -m pytest tests/test_operational_governance_mvp.py tests/test_database_upgrades.py` (from **`backend/`**) — passed.
- **Administration-tool — operational governance manage page:** `python -m pytest tests/test_manage_operational_governance.py` (from **`administration-tool/`**) — passed.
- **Docify hub tools:** `python -m pytest "'fy'-suites/docify/tools/tests" -q` (from repo root, after **`pip install -e .`**) — eight tests, including **`parse_open_doc_ids`** / **`open-doc --input`** paths that do not require monorepo **`pyproject.toml`** discovery; passed in CI per [`.github/workflows/docify-skills-validate.yml`](.github/workflows/docify-skills-validate.yml).
- **Contractify hub tools:** `python -m pytest "'fy'-suites/contractify/tools/tests" -q` (from repo root, after **`pip install -e .`**) — twenty-six tests; local audit JSON under **`'fy'-suites/contractify/reports/*.json`** (single segment) is gitignored; full hermetic fixtures live under **`'fy'-suites/contractify/reports/committed/`**.
- **fy_platform and Wave 1 portability bundle:** `python -m pytest "'fy'-suites/fy_platform/tests" "'fy'-suites/despaghettify/tools/tests/test_hub_cli_portability.py" "'fy'-suites/docify/tools/tests/test_python_documentation_audit.py" "'fy'-suites/postmanify/tools/tests/test_openapi_postman.py" "'fy'-suites/contractify/tools/tests/test_hub_cli.py" -q` (from repo root, after **`pip install -e .`**) — **27** tests covering manifest resolution on fixtures, shared envelope golden serialization, bootstrap/validate CLI, deprecation matrix invariants, Docify/Postmanify/Contractify envelope and deprecation sidecars, and **Despaghettify** **`check`** on a non–World-of-Shadows synthetic tree.

---

## [0.6.3] - 2026-04-13

**Summary:** Hardened the **multi-component test runner** at [`tests/run_tests.py`](tests/run_tests.py): **environment checks** probe the stacks each suite needs (Flask / SQLAlchemy for backend-shaped trees, FastAPI for the world-engine tree, editable **story_runtime_core** and **ai_stack** for the AI stack suite) so a bare **pytest** install cannot falsely pass before collection fails; **`--scope`** maps to **pytest markers** for **administration-tool** and **world-engine** as well as backend, writers-room, and improvement (with clear behaviour when a marker is not registered); **`--suite all`** uses **`ALL_SUITE_SEQUENCE`** so **writers_room** / **improvement** are not double-run; **`--quick`** skips pre-run **collect-only** stats by default (**`--stats`** to force); **`show_test_stats`** reflects subprocess **exit codes**; **`--continue-on-failure`** overrides quick-mode stop-after-first-failing-suite; **coverage** uses explicit **`--cov=`** targets and named orchestrator paths instead of ad-hoc **`--cov=.`** for engine, administration, database, and **ai_stack** runs. **Operator docs:** rewritten [`tests/TESTING.md`](tests/TESTING.md), new [`docs/testing/COVERAGE_SEMANTICS.md`](docs/testing/COVERAGE_SEMANTICS.md), and links from [`docs/testing/INDEX.md`](docs/testing/INDEX.md). **Optional lanes:** manual **Compose** workflow [`.github/workflows/compose-smoke.yml`](.github/workflows/compose-smoke.yml) with [`tests/smoke/compose_smoke/`](tests/smoke/compose_smoke/); **Playwright** scaffold under [`tests/e2e/`](tests/e2e/) (not driven by **`run_tests.py`**). **Postmanify** suite **`'fy'-suites/postmanify/`** with **`postmanify`** CLI (`plan`, `generate`) refreshes **`postman/WorldOfShadows_Complete_OpenAPI.postman_collection.json`** and **`postman/suites/`** from **`docs/api/openapi.yaml`**, Cursor skill **`postmanify-sync`**, root **`pyproject.toml`** **PyYAML** dependency and **`postmanify`** script; **legacy** hand Postman JSON collections **removed** in favour of that OpenAPI-driven layout (**`postman/README.md`**, **`docs/api/REFERENCE.md`**, **`docs/api/POSTMAN_COLLECTION.md`**, forum workflow doc pointers). **Despaghettify** hub moved from root **`despaghettify/`** to **`'fy'-suites/despaghettify/`** with **`hub_cli.py`** to avoid import shadowing; **`AGENTS.md`**, **`CONTRIBUTING.md`**, **`docs/dev/contributing.md`**, **`.github/workflows/despaghettify-skills-validate.yml`**, and hub Markdown updated for paths. **Docify** documentation hub moved from root **`docify/`** to **`'fy'-suites/docify/`** (suite catalog row in **`'fy'-suites/README.md`**, **`AGENTS.md`**, **CI** path filters, task Markdown, and **Cursor** skill copies); **CLIs** gain optional **Google-style** docstring drafting (**`--emit-google-docstring`**, **`--apply-docstring`**) and stricter existing-docstring checks (**`--google-docstring-audit`**, **`--docstring-max-line`**) with **72**-column wrapping and clearer **`Args:`** / **`Returns:`** handling. **Backend** tests: **`assert True`** stubs replaced, **FK** assertions in upgrades tests, parallel read thread coverage and renames in cross-service consistency; **world-engine** contract tests assert richer health JSON; **CI** [`.github/workflows/ai-stack-tests.yml`](.github/workflows/ai-stack-tests.yml) **imports `ai_stack.langgraph.langgraph_runtime`** before **pytest**. **Docify / AI stack:** **`'fy'-suites/docify/tools/strip_ai_stack_docstring_placeholders.py`** rewrites **leading** module, class, and function docstrings under **`ai_stack/`** (skips **`ai_stack/tests/`**) to strip leftover **Google-style** bulk-placeholder prose (including split **`name: TODO: document`** / **``name``** **`(type…)`** lines, multi-line **`Returns:`** boilerplate, and **Replace this paragraph** stubs); **`python_docstring_synthesize`** no longer emits literal **TODO** wording in default **Args** / **Returns** / summary lines; contributor-facing **wave plan** in [`docs/dev/ai_stack-documentation-waves.md`](docs/dev/ai_stack-documentation-waves.md). **Suite hubs** ship as a committed **`'fy'-suites/`** tree (**Despaghettify**, **Docify**, **Postmanify** — see **`'fy'-suites/README.md`**); **Cursor** copies under **`.cursor/skills/`** include **docify-orchestrate**, **docify-synthesize**, and **postmanify-sync** (skills **README** refreshed). **Backend** [`backend/app/runtime/ai_turn_decision_helpers.py`](backend/app/runtime/ai_turn_decision_helpers.py) gains structured **Google-style** docstrings for persisted-delta and decision-log helpers; **new** [`backend/tests/services/`](backend/tests/services/) cases cover **AI stack closure cockpit** parsing, **inspector turn projection** (service + semantic sections), and **user update** guard rails. **Forum** [`docs/forum/ModerationWorkflow.md`](docs/forum/ModerationWorkflow.md) points at the OpenAPI-driven Postman layout. **Fixed** duplicate **`test_game_service`** collection id by renaming [`backend/tests/test_play_service_client.py`](backend/tests/test_play_service_client.py); **coverage parity** across **`pytest.ini`** files, **administration-tool** **`.coveragerc`**, and orchestrator **database** suite **`--cov-fail-under`** behaviour.
...
- **Docify — Google-style docstring draft (single callable):** `'fy'-suites/docify/tools/python_docstring_synthesize.py` — **`--emit-google-docstring`** with **`--function`** prints or writes (via **`--apply-docstring`**) a wrapped draft with **`Args:`**, **`Returns:`**, and a **`Type:`**-style line when the signature has parameters or a non-**`None`** return annotation; narrative lines target **72** characters for readability.
- **Docify — optional docstring layout audit:** `'fy'-suites/docify/tools/python_documentation_audit.py` — **`--google-docstring-audit`** (and **`--docstring-max-line`**) report missing **`Args:`** / **`Returns:`** sections, a missing type line after **`Returns:`**, and overlong docstring lines on symbols that already have text.
- **Docify — regression tests:** [`tests/tools/test_python_docstring_synthesize.py`](tests/tools/test_python_docstring_synthesize.py) (Google draft smoke + audit hand-off; **`importlib`** entrypoint under **`'fy'-suites/docify/tools/`**); [`tests/tools/test_python_documentation_audit.py`](tests/tools/test_python_documentation_audit.py) (indented **`Args:`** / **`Returns:`** acceptance under the layout audit; same path layout).
- **Docify — strip bulk docstring placeholders in `ai_stack`:** **`'fy'-suites/docify/tools/strip_ai_stack_docstring_placeholders.py`** — AST-scoped rewrites of **leading** docstrings only (no **`# TODO`** comments); optional **`--dry-run`**; run from repo root via **`python "./'fy'-suites/docify/tools/strip_ai_stack_docstring_placeholders.py"`**.
- **`'fy'-suites/` hub tree (committed):** relocates **Despaghettify**, **Docify**, and **Postmanify** (tools, task Markdown, **`superpowers/`** skills, sync/validate scripts, suite **README**) under one repo root directory; **`pyproject.toml`** discovers **`despaghettify`** from that layout.
- **Developer guide — `ai_stack` doc waves:** [`docs/dev/ai_stack-documentation-waves.md`](docs/dev/ai_stack-documentation-waves.md) — wave sizing, suggested module order, definition of done, and **Docify** audit commands for iterative docstring cleanup.
- **Postman — OpenAPI-driven bundle:** **`postman/WorldOfShadows_Complete_OpenAPI.postman_collection.json`**, **`postman/postmanify-manifest.json`**, **`postman/suites/`**, and **`postman/generated/`** (layout described in **`postman/README.md`**).
- **Cursor — suite skill copies:** **`.cursor/skills/docify-orchestrate/`**, **`.cursor/skills/docify-synthesize/`**, **`.cursor/skills/postmanify-sync/`**; **`.cursor/skills/README.md`** lists the new entries.
- **Backend — new service tests:** [`backend/tests/services/test_ai_stack_closure_cockpit_parsing.py`](backend/tests/services/test_ai_stack_closure_cockpit_parsing.py), [`test_inspector_turn_projection_service.py`](backend/tests/services/test_inspector_turn_projection_service.py), [`test_inspector_turn_projection_sections_semantic.py`](backend/tests/services/test_inspector_turn_projection_sections_semantic.py), [`test_user_service_update_guards.py`](backend/tests/services/test_user_service_update_guards.py).
...
- **Multi-component test runner:** [`tests/run_tests.py`](tests/run_tests.py) — **`check_environment(suites)`** runs subprocess import probes per selected suite; **`--scope`** drives **`-m`** for administration, engine, backend, writers_room, and improvement; **`show_test_stats`** honours the same scope for **collect-only** and returns success/failure from subprocess **exit codes**; **`ALL_SUITE_SEQUENCE`** for **`--suite all`** (six suites, no duplicate writers_room/improvement runs); **`--quick`** / **`--stats`** / **`--continue-on-failure`** (orchestrator stops after first failing suite in quick mode unless **`--continue-on-failure`**); **`pytest-cov`** arguments use **`_cov_sources_for_suite()`** / **`_append_cov_flags()`** with explicit module lists for **administration-tool** and filesystem paths for **backend**, **frontend**, **engine**, **database**, and **ai_stack** trees; module docstring, **`--help`**, and epilog aligned with **`python tests/run_tests.py`** from repo root.
...
- **AI Stack CI:** [`.github/workflows/ai-stack-tests.yml`](.github/workflows/ai-stack-tests.yml) — explicit **`import ai_stack.langgraph.langgraph_runtime`** step before **pytest**.
- **Root `.gitignore`:** ignore **`tests/e2e/node_modules/`**, **`tests/e2e/playwright-report/`**, **`tests/e2e/test-results/`**.
- **Docify hub location and discovery:** the full hub tree now lives under **`'fy'-suites/docify/`** (no root **`docify/`** directory). **`'fy'-suites/Readme.md`** lists **`docify/`** in the suite catalog with copy-paste **`python "./'fy'-suites/docify/tools/…"`** commands; [`AGENTS.md`](AGENTS.md) documents the same layout and sync rule (**`python "./'fy'-suites/docify/tools/sync_docify_skills.py"`** after editing **`'fy'-suites/docify/superpowers/*/SKILL.md`**). Hub **README**, task Markdown, and **`superpowers/`** links use deeper **`../../`** paths to the repo root; **`.cursor/skills/docify-*`** copies refresh from the new canonical tree.
- **Docify tooling internals:** **`'fy'-suites/docify/tools/sync_docify_skills.py`** and **`validate_docify_skill_paths.py`** resolve the repository root with **`Path(__file__).resolve().parents[3]`** and scan **`DOCIFY_ROOT / "superpowers"`**; **`python_documentation_audit.py`** and **`python_docstring_synthesize.py`** use the same root depth. **CI:** [`.github/workflows/docify-skills-validate.yml`](.github/workflows/docify-skills-validate.yml) and [`.github/workflows/backend-tests.yml`](.github/workflows/backend-tests.yml) watch **`'fy'-suites/docify/**`** instead of **`docify/**`**.
- **Docify default audit scope:** the **`'fy'-suites/docify`** tree is **not** included in the audit CLI’s default **`--root`** set so routine full-tree runs do not self-report missing docstrings on the helper scripts; pass **`--root 'fy'-suites/docify`** when you want that slice (documented in the hub **README** and **`AGENTS.md`**).
- **Docify — `python_docstring_synthesize`:** default **Google-style** section and summary lines use neutral maintainer-facing wording instead of literal **TODO** / “replace this” scaffolding where those templates apply.
- **AI stack — docstrings:** leading docstrings across **`ai_stack/`** (outside **`tests/`**) were normalized with the strip tool so shipped **Args** / **Returns** blocks no longer read as unfinished checklist items; contracts remain in signatures and implementation.
- **Backend — AI turn decision helpers:** [`backend/app/runtime/ai_turn_decision_helpers.py`](backend/app/runtime/ai_turn_decision_helpers.py) — **Google-style** docstrings for delta mapping and **`AIDecisionLog`** construction (persistence-oriented helpers).
...
- **Legacy Postman environments:** removed **`postman/WorldOfShadows_Docker.postman_environment.json`**, **`WorldOfShadows_Local.postman_environment.json`**, and **`WorldOfShadows_Test.postman_environment.json`** (superseded by the OpenAPI-driven collection workflow and current operator docs).
...
- **Docify — Google layout audit heuristics:** indented **`Args:`** and **`Returns:`** headers inside docstrings are recognised; the text after **`Returns:`** is split with a line-aware regex so the first non-blank line can be validated as a **`Type:`**-style label.

---

## [0.6.2] - 2026-04-12

**Summary:** **Despaghettify** hub and tooling after **`[0.6.1]`**: file-backed **autonomous** session **CLI** (`autonomous-init`, **`backlog-implement`** slice checkpoints, **`backlog-solve`** only after closing a **DS-*** row, `main-check` / `main-solve`), **`metrics_bundle`** heuristic **v2** with literal **Anteil** rates, German **`plain_language_de`** lines, and **Metrik A**; **WOS-STRICT-2** trigger weights and a tighter **C1** bar with regenerated **`spaghetti-setup.json`**; **clean** / **reset** task docs, **Mermaid** phase rules, **add-task-to-meet-trigger** playbook, **wave plan** emit/validate and CI smoke paths; **Cursor** skill copy workflow and path validation. **Structure** input list repopulated from **`spaghetti-reset-task`** (**DS-001..DS-005**). Backend and **AI stack** companion modules (improvement experiment pipeline types, lore/progression/session helpers, **LangGraph** research phases, an early **GoC** scene-candidate helper later superseded by semantic move contracts, inspector projection shared logic). **Administration-tool** and **GoC** test suites refactored; **N8N** webhook tests expanded. **Removed** local workstream session files under **`backend_runtime_services`** when running the hub clean/reset recipe (recover from **Git** or **CI** if you still need those filenames). **Technical audit resolution (same release):** reusable governance pack under **`docs/governance/audit_resolution/`** (master prompt, case input, living state, closure checklist, **Part L1** frozen hygiene criteria, findings **F-L1a** / **F-L1b**); **Accepted ADR-0002** (Appendix A session surface inventory + retirement decision gate) and **Accepted ADR-0003** (canonical GoC scene identity in code with CI single-source guard); **M2** supported test-invocation matrix, root **`Makefile`**, and **`scripts/run_*`** helpers; World Engine **durable story sessions** (JSON store, reload on start, optional **`content_provenance`**, per-session **threading** lock for turns); **aligned** backend compiler plus World Engine **narrative commit** scene row keys (**`id`** and **`scene_id`**).

### Added

- **Despaghettify autonomous machine guards:** `python -m despaghettify.tools autonomous-init` | `autonomous-advance` | `autonomous-status` | `autonomous-verify` (state under `despaghettify/state/artifacts/autonomous_loop/`, schema [`despaghettify/tools/schemas/autonomous_state.schema.json`]('fy'-suites/despaghettify/tools/schemas/autonomous_state.schema.json)); [`spaghetti-autonomous-agent-task.md`]('fy'-suites/despaghettify/spaghetti-autonomous-agent-task.md) and router skill mandate CLI checkpoints; [`despaghettify/spaghetti-setup.json`]('fy'-suites/despaghettify/spaghetti-setup.json) mirrors numeric policy for tools; **`metrics-emit`**, **`trigger-eval`**, and **`check --with-metrics`** ([`despaghettify/tools/metrics_bundle.py`]('fy'-suites/despaghettify/tools/metrics_bundle.py)); unit tests under [`despaghettify/tools/tests/`]('fy'-suites/despaghettify/tools/tests/); CI steps in [`.github/workflows/despaghettify-skills-validate.yml`](.github/workflows/despaghettify-skills-validate.yml). Run **`python "./'fy'-suites/despaghettify/tools/sync_despag_skills.py"`** after skill edits.
- **Despaghettify autonomous macro loop:** [`despaghettify/spaghetti-autonomous-agent-task.md`]('fy'-suites/despaghettify/spaghetti-autonomous-agent-task.md) defines **check → solve (per DS) → check** until [`spaghetti-setup.md`]('fy'-suites/despaghettify/spaghetti-setup.md) conditions clear and no open **DS-*** remain, with **in-scope** hard stops and **anti-stall**; router skill [`despaghettify/superpowers/despaghettify-autonomous-agent/SKILL.md`]('fy'-suites/despaghettify/superpowers/despaghettify-autonomous-agent/SKILL.md); [`despaghettify/README.md`]('fy'-suites/despaghettify/README.md), [`despaghettify/superpowers/README.md`]('fy'-suites/despaghettify/superpowers/README.md), [`despaghettify/superpowers/despaghettify-orchestrate/SKILL.md`]('fy'-suites/despaghettify/superpowers/despaghettify-orchestrate/SKILL.md), [`despaghettify/superpowers/references/WORKFLOW.md`]('fy'-suites/despaghettify/superpowers/references/WORKFLOW.md), [`AGENTS.md`](AGENTS.md), and [`despaghettify/superpowers/references/CLI.md`]('fy'-suites/despaghettify/superpowers/references/CLI.md) updated; run **`python "./'fy'-suites/despaghettify/tools/sync_despag_skills.py"`** after pull.
- **Despaghettify numeric policy file:** [`despaghettify/spaghetti-setup.md`]('fy'-suites/despaghettify/spaghetti-setup.md) — single editable source for **C1..C7** trigger bars, **M7** category weights, and derived **`M7_ref`**; [`spaghetti-check-task.md`]('fy'-suites/despaghettify/spaghetti-check-task.md), the implementation input list, **EMPTY** template, reset/solve/add-task docs, hub **README**, and router **skills** reference **setup** for numbers and the check task for procedure (run **`python "./'fy'-suites/despaghettify/tools/sync_despag_skills.py"`** after skill edits).
- **Despaghettify router skills (clean / reset):** [`despaghettify/superpowers/despaghettify-clean/SKILL.md`]('fy'-suites/despaghettify/superpowers/despaghettify-clean/SKILL.md) and [`despaghettify/superpowers/despaghettify-reset/SKILL.md`]('fy'-suites/despaghettify/superpowers/despaghettify-reset/SKILL.md) mirror **check** / **solve** / **add-task** — each points at a single canonical task file (`spaghetti-clean-task.md`, `spaghetti-reset-task.md`). [`despaghettify/superpowers/README.md`]('fy'-suites/despaghettify/superpowers/README.md), [`despaghettify/superpowers/despaghettify-orchestrate/SKILL.md`]('fy'-suites/despaghettify/superpowers/despaghettify-orchestrate/SKILL.md), and [`despaghettify/superpowers/references/WORKFLOW.md`]('fy'-suites/despaghettify/superpowers/references/WORKFLOW.md) updated; run **`python "./'fy'-suites/despaghettify/tools/sync_despag_skills.py"`** to refresh **`.cursor/skills/`**.
- **Despaghettify CLI:** [`hub_cli.py`]('fy'-suites/despaghettify/tools/hub_cli.py) — invoke as **`python -m despaghettify.tools`** (or the script path); `check` (JSON report: AST stats via `collect_ast_stats()`, `ds005`, builtins + runtime greps), `open-ds` (open **DS-*** ids from the input list), `solve-preflight --ds DS-nnn` (exit **0** if open, **2** if missing/closed; JSON includes **`wave_sizing`** heuristics when open), **`wave-plan-validate --file`** (schema for persisted **`wave_plan.json`**). In-repo skills under [`despaghettify/superpowers/`]('fy'-suites/despaghettify/superpowers/) reference this runner as the automation entry point. Schema: [`WAVE_PLAN_SCHEMA.md`]('fy'-suites/despaghettify/superpowers/references/WAVE_PLAN_SCHEMA.md); example: [`despaghettify/tools/fixtures/despag_wave_plan_example_valid.json`]('fy'-suites/despaghettify/tools/fixtures/despag_wave_plan_example_valid.json).
- **Cursor skill discovery:** root [`.gitignore`](.gitignore) now allows **`.cursor/skills/**`** (other `.cursor/*` stays ignored). [`despaghettify/tools/sync_despag_skills.py`]('fy'-suites/despaghettify/tools/sync_despag_skills.py) copies `despaghettify/superpowers/*/SKILL.md` → `.cursor/skills/<name>/SKILL.md` (`--check` for CI drift). Added [`.cursor/skills/README.md`](.cursor/skills/README.md) and root [`AGENTS.md`](AGENTS.md) pointer.
- **Despaghettify skill path validation:** [`despaghettify/tools/validate_despag_skill_paths.py`]('fy'-suites/despaghettify/tools/validate_despag_skill_paths.py) checks links / backtick repo paths under `despaghettify/superpowers/**/*.md`; workflow [`.github/workflows/despaghettify-skills-validate.yml`](.github/workflows/despaghettify-skills-validate.yml); optional pre-commit snippet in [`despaghettify/superpowers/references/VALIDATION.md`]('fy'-suites/despaghettify/superpowers/references/VALIDATION.md).
- **Despaghettify single source of truth:** [`spaghetti-check-task.md`]('fy'-suites/despaghettify/spaghetti-check-task.md) (and sibling solve/add-task docs) declare **sole** procedure per track; **numeric** trigger policy is in [`spaghetti-setup.md`]('fy'-suites/despaghettify/spaghetti-setup.md). Hub **skills** are **routers** only (`.cursor/skills` refreshed via [`despaghettify/tools/sync_despag_skills.py`]('fy'-suites/despaghettify/tools/sync_despag_skills.py)).
- **Despaghettify / Cursor on Windows:** [`despaghettify/superpowers/references/CURSOR_ACTIVATION.md`]('fy'-suites/despaghettify/superpowers/references/CURSOR_ACTIVATION.md) documents **copy-only** sync (no symlinks / `mklink`); [`.cursor/skills/README.md`](.cursor/skills/README.md) and [`AGENTS.md`](AGENTS.md) aligned.
...
- **God of Carnage scene-candidate legacy path (later removed):** the early
  ...
  contract; do not add new callers or documentation for keyword scene routing.
- **Audit resolution governance (three-layer):** [`docs/governance/audit_resolution/`](docs/governance/audit_resolution/) — universal master prompt, **World of Shadows** technical audit **2026-04-12** case input, living state and **CAPA** register, closure packet checklist; [`docs/governance/README.md`](docs/governance/README.md) indexes **ADRs** and the bundle. ADR acceptance and enforcement details are summarized in the **Audit closure H2 / H3 / M2 / L1** bullet below.
- **World Engine durable story sessions:** [`world-engine/app/story_runtime/story_session_store.py`](world-engine/app/story_runtime/story_session_store.py) JSON store under **`STORY_SESSION_STORE_DIR`** ([`world-engine/app/config.py`](world-engine/app/config.py)), reload on process start from [`world-engine/app/main.py`](world-engine/app/main.py), optional **`content_provenance`** on **`POST /api/story/sessions`** ([`world-engine/app/api/http.py`](world-engine/app/api/http.py)); persistence round-trip in [`world-engine/tests/test_story_session_persistence.py`](world-engine/tests/test_story_session_persistence.py).
- **World Engine concurrent story turns:** per-session **`threading.Lock`** around turn execution in [`world-engine/app/story_runtime/manager.py`](world-engine/app/story_runtime/manager.py); stress coverage in [`world-engine/tests/test_story_runtime_narrative_commit.py`](world-engine/tests/test_story_runtime_narrative_commit.py).
- **Regression tests — compiler-shaped narrative commit seam:** **`scene_id`-only** runtime projection rows still commit a legal explicit-command transition ([`world-engine/tests/test_story_runtime_narrative_commit.py`](world-engine/tests/test_story_runtime_narrative_commit.py)); God of Carnage compile output asserts matching **`id`** and **`scene_id`** on every scene row ([`backend/tests/content/test_content_compiler.py`](backend/tests/content/test_content_compiler.py)).
- **Audit closure H2 / H3 / M2 / L1:** [ADR-0002](docs/ADR/adr-0002-backend-session-surface-quarantine.md) **Accepted** with Appendix A inventory + retirement **decision gate**; [ADR-0003](docs/ADR/adr-0003-scene-identity-canonical-surface.md) **Accepted** with **Option A** source-of-truth in [`ai_stack/goc_scene_identity.py`](ai_stack/goc_scene_identity.py), [`ai_stack/tests/test_goc_scene_identity.py`](ai_stack/tests/test_goc_scene_identity.py), and [`tools/verify_goc_scene_identity_single_source.py`](tools/verify_goc_scene_identity_single_source.py) (CI in [`.github/workflows/ai-stack-tests.yml`](.github/workflows/ai-stack-tests.yml)); **supported test invocation matrix** in [`docs/dev/contributing.md`](docs/dev/contributing.md), root [`Makefile`](Makefile), and [`scripts/run_*`](scripts/); state **Part L1** frozen criteria and findings F-L1a / F-L1b in [`audit_resolution_state_world_of_shadows.md`](docs/governance/audit_resolution/audit_resolution_state_world_of_shadows.md).
- **Root `.gitignore`:** ignore **`world-engine/app/var/story_sessions/`** so local JSON session files from the World Engine do not clutter **`git status`**.
- **Despaghettify solve:** [`spaghetti-solve-task.md`]('fy'-suites/despaghettify/spaghetti-solve-task.md) adds **wave sizing** (bound **N**, simple-task-sized slices), **persisted wave plan** when **N > 1**, **session-limited autonomy**, and **resume after interruption** (work log + continue at **k+1**); router skill pointer updated. Follow-up: **machine `wave_plan.json`**, **`wave-plan-validate`**, **`solve-preflight` → `wave_sizing`**, **merge/rebase between waves**, and clearer **same-session** autonomy wording. CI workflow runs **`python "./'fy'-suites/despaghettify/tools/sync_despag_skills.py" --check`**, **`wave-plan-validate`** on the fixture, and **`solve-preflight --ds DS-016`** smoke.

### Changed

- **Repository language — single canonical policy:** [`docs/dev/contributing.md`](docs/dev/contributing.md) heading **Repository language** (`#repository-language`) is the **only** place the full strict-English rules live (project-wide maintainer- and operator-facing text, with explicit carve-out for in-world narrative under `content/modules/`). [`AGENTS.md`](AGENTS.md) and [`CONTRIBUTING.md`](CONTRIBUTING.md) point there and do **not** restate the policy. [`despaghettify/state/EXECUTION_GOVERNANCE.md`]('fy'-suites/despaghettify/state/EXECUTION_GOVERNANCE.md), [`despaghettify/README.md`]('fy'-suites/despaghettify/README.md), [`despaghettify/despaghettification_implementation_input.md`]('fy'-suites/despaghettify/despaghettification_implementation_input.md), [`despaghettify/state/README.md`]('fy'-suites/despaghettify/state/README.md), and Despaghettify task Markdowns (`spaghetti-*.md`, [`spaghetti-setup.md`]('fy'-suites/despaghettify/spaghetti-setup.md)) link **up** to that anchor and keep **procedure-only** language scope where needed, avoiding parallel copies that could drift.
- **Despaghettify `metrics_bundle` (heuristic v2):** [`despaghettify/tools/metrics_bundle.py`]('fy'-suites/despaghettify/tools/metrics_bundle.py) replaces **v1** flat caps with **saturating** ``100*(1-exp(-…))`` curves capped **below 100** so proxy scores never read as “totally spaghetti”. **C4** uses **>50-line** share vs **total functions**; **C6** uses **>100-line** share — no longer the same **`L₅₀`** ladder as C4. Re-run **`python -m despaghettify.tools check --with-metrics`** and copy **`metrics_bundle`** into § *Latest structure scan* after upgrading.
- **Despaghettify `metrics_bundle` — echte Anteile + deutsche Lesesätze:** [`despaghettify/tools/metrics_bundle.py`]('fy'-suites/despaghettify/tools/metrics_bundle.py) ergänzt **`literal_rates`** (z. B. Anteil der gemessenen Funktionen mit **>100** bzw. **>50** AST-Zeilen, Verschachtelung **≥6**) und **`plain_language_de`** (ein Satz pro **C1..C7**; **C1**/**C5** erklären fehlende %-Messung). **`category_scores`** bleiben die **Trigger-Skala** gegen die Balken in **`spaghetti-setup.json`**.
- **Despaghettify `metrics_bundle` — Metrik A:** [`despaghettify/tools/metrics_bundle.py`]('fy'-suites/despaghettify/tools/metrics_bundle.py) liefert **`metric_a`** (`share_weighted_m7`): **gleiche Gewichte** wie **M7**, angewandt auf die **Anteil-Vektoren** aus **`literal_rates`** (Berichtskennzahl; **kein** Trigger-Ersatz für **`m7`**).
- **Despaghettify hub CLI ergonomics:** canonical invocation **`python -m despaghettify.tools`** via [`despaghettify/tools/__main__.py`]('fy'-suites/despaghettify/tools/__main__.py) (prepends repo root to `sys.path`); optional root [`pyproject.toml`](pyproject.toml) package **`world-of-shadows-hub`** adds console scripts **`despag-check`** and **`wos-despag`** after **`pip install -e .`**. [`spaghetti_ast_scan.py`]('fy'-suites/despaghettify/tools/spaghetti_ast_scan.py) resolves **`ROOTS`** from `__file__` so **CWD** need not be the repo root. **Deprecated** thin shims restore old paths: [`tools/spaghetti_ast_scan.py`](tools/spaghetti_ast_scan.py), [`tools/ds005_runtime_import_check.py`](tools/ds005_runtime_import_check.py). Docs ([`CLI.md`]('fy'-suites/despaghettify/superpowers/references/CLI.md), [`AGENTS.md`](AGENTS.md), solve/check tasks), [`.github/workflows/despaghettify-skills-validate.yml`](.github/workflows/despaghettify-skills-validate.yml), and router skills updated; run **`python "./'fy'-suites/despaghettify/tools/sync_despag_skills.py"`** after skill edits.
- **Despaghettify tooling layout:** hub scripts (`despaghettify.py`, `sync_despag_skills.py`, `validate_despag_skill_paths.py`), **`spaghetti_ast_scan.py`**, **`ds005_runtime_import_check.py`**, and the **`wave_plan`** example fixture live under **`despaghettify/tools/`** (those two runners were removed from repo root **`tools/`**). `despaghettify.py check`, task docs, templates, and wave-plan fixtures use the new paths; **`ds005`** resolves `backend/` from repo root via the script location. Documentation, CI, and Cursor skill copies updated accordingly.
- **Despaghettify wave plan / CI:** `solve-preflight` smoke uses **first `open-ds` id**, else **`--override-input-list`** stub **DS-990** under `despaghettify/tools/fixtures/` (no hard dependency on a live DS row). New **`wave_plan_emit.py`** (`json2md` / `md2json` / table round-trip). **`wave-plan-validate`** gains **`--check-primary-paths`** and **`--gate-prefix-allowlist`**. **`wave_plan.json`** may include optional **`completed_wave_ids`** and **`next_index`** (schema + solve task).
- **Despaghettify / spaghetti check:** Mermaid `flowchart` (or `graph`) under § *Recommended implementation order* is **mandatory** whenever the phase table has real rows ([`spaghetti-check-task.md`]('fy'-suites/despaghettify/spaghetti-check-task.md) §3); hub diagrams use **single-line** node labels `["phase · DS-ID · short hook"]` for stable rendering (GitHub / MkDocs). **Parallel** waves use fork/join (e.g. `3a`/`3b`). [`despaghettification_implementation_input.md`]('fy'-suites/despaghettify/despaghettification_implementation_input.md) example updated; template and reset task aligned.
- **Despaghettify — structure scan presentation:** [`spaghetti-check-task.md`]('fy'-suites/despaghettify/spaghetti-check-task.md) §1 requires **C1..C7** and **M7** as **percent** text, **AST telemetry** in the main scan table **and** an extra row **under C7** in the § *Score M7* **Symbol | Meaning | Value** table (not in the trigger-policy table); live input and **EMPTY** template updated.
- **Despaghettify — DS rows:** **Information input list** **pattern** column leads with **M7 trigger category** symbols (**`C3 ·`** …, optional multi **`C2 · C3 ·`**) per [`spaghetti-check-task.md`]('fy'-suites/despaghettify/spaghetti-check-task.md) §2; **C7** added to that doc’s per-category table; live DS table and **EMPTY** template aligned.
- **Despaghettify:** new hub playbook [`spaghetti-add-task-to-meet-trigger.md`]('fy'-suites/despaghettify/spaghetti-add-task-to-meet-trigger.md) — parameter **C1..C7**, propose **DS-*** waves to lower that category, full **Recommended implementation order** re-pass (parallelism, deps, Mermaid); linked from [`despaghettify/README.md`]('fy'-suites/despaghettify/README.md).
- **Despaghettify input list:** example **`spaghetti-add-task-to-meet-trigger` (C1)** pass — **DS-015** (`**C1 ·**` runtime import-seam hardening), phased **before** DS-014; phases **4a/4b/5** and Mermaid updated in [`despaghettification_implementation_input.md`]('fy'-suites/despaghettify/despaghettification_implementation_input.md).
- **Despaghettify:** added [`despaghettify/spaghetti-clean-task.md`]('fy'-suites/despaghettify/spaghetti-clean-task.md) — removes all session files under `despaghettify/state/artifacts/workstreams/<slug>/pre|post/` and recreates empty directories per [`WORKSTREAM_INDEX.md`]('fy'-suites/despaghettify/state/WORKSTREAM_INDEX.md); [`spaghetti-reset-task.md`]('fy'-suites/despaghettify/spaghetti-reset-task.md) now starts with this clean as **Step 0**; [`despaghettify/README.md`]('fy'-suites/despaghettify/README.md) documents the split.
- **Despaghettify trigger policy — WOS-STRICT-2 weights and tighter C1 bar:** [`despaghettify/spaghetti-setup.md`]('fy'-suites/despaghettify/spaghetti-setup.md) raises the **C1** per-category bar to **2%**, documents the **WOS-STRICT-2 (B)** weight preset (higher emphasis on **C1** and **C5**), recomputes **`M7_ref`**, and clarifies **strict** gates versus **calibration**; [`despaghettify/spaghetti-setup.json`]('fy'-suites/despaghettify/spaghetti-setup.json) regenerated with **`setup-sync`**; [`despaghettify/tools/tests/test_spaghetti_setup_audit.py`]('fy'-suites/despaghettify/tools/tests/test_spaghetti_setup_audit.py) expectations updated.
- **Structure hub — reset-then-check snapshot:** [`despaghettification_implementation_input.md`]('fy'-suites/despaghettify/despaghettification_implementation_input.md) repopulated after **`spaghetti-reset-task`** (latest scan table, **DS-001..DS-005**, recommended order + Mermaid, progress log row); refreshed machine evidence under [`despaghettify/reports/reset_check_with_metrics.json`]('fy'-suites/despaghettify/reports/reset_check_with_metrics.json) and [`reset_ast_scan_capture.txt`]('fy'-suites/despaghettify/reports/reset_ast_scan_capture.txt); brief notes in [`WORKSTREAM_AI_STACK_STATE.md`]('fy'-suites/despaghettify/state/WORKSTREAM_AI_STACK_STATE.md) and [`WORKSTREAM_BACKEND_RUNTIME_AND_SERVICES_STATE.md`]('fy'-suites/despaghettify/state/WORKSTREAM_BACKEND_RUNTIME_AND_SERVICES_STATE.md).
- **Runtime narrative and evidence services (incremental splits):** [`lore_direction_context.py`](backend/app/runtime/lore_direction_context.py), [`progression_summary.py`](backend/app/runtime/progression_summary.py), [`session_history.py`](backend/app/runtime/session_history.py), [`inspector_projection_service.py`](backend/app/services/inspector_projection_service.py), [`inspector_projection_coverage_health.py`](backend/app/services/inspector_projection_coverage_health.py), [`ai_stack_evidence_service.py`](backend/app/services/ai_stack_evidence_service.py), and [`ai_stack_evidence_session_bundle.py`](backend/app/services/ai_stack_evidence_session_bundle.py) pull logic into companion modules; [`package_classification.py`](backend/app/runtime/package_classification.py) lists new roots.
- **Administration-tool automated tests:** large consistency pass across error-page, proxy, and route-contract suites under **`administration-tool/tests/`** (formatting, fixtures, or isolation — see diffs for detail).
- **God of Carnage regression tests:** updates across **`test_goc_*`** files for fixture and seam changes.
- **N8N webhook trigger tests:** refactors and additional cases in [`backend/tests/test_n8n_trigger.py`](backend/tests/test_n8n_trigger.py).
- **Backend content compiler and World Engine narrative commit:** each compiled scene row now carries **both** **`id`** and **`scene_id`** with the same value ([`backend/app/content/compiler/compiler.py`](backend/app/content/compiler/compiler.py)); the narrative commit resolver accepts either key on **`runtime_projection.scenes`** and logs when rows are not machine-readable ([`world-engine/app/story_runtime/commit_models.py`](world-engine/app/story_runtime/commit_models.py)). [**Canonical runtime contract**](docs/technical/architecture/canonical_runtime_contract.md) documents the story **`runtime_projection`** expectation at the backend–engine seam.
- **Contributor bootstrap — monorepo Python:** new **Monorepo Python bootstrap** subsection in [`docs/dev/contributing.md`](docs/dev/contributing.md) (run **`pytest`** from each service root; avoid a single mixed **`PYTHONPATH`** across **`backend/`** and **`world-engine/`** so the top-level **`app`** package does not resolve to the wrong tree). Adds the **Supported test invocation matrix (M2)** (supported vs forbidden cwd for backend and world-engine suites) with cross-links from [`backend/README.md`](backend/README.md) and [`world-engine/README.md`](world-engine/README.md).

### Removed

- **Local workstream session artefacts:** files under `despaghettify/state/artifacts/workstreams/backend_runtime_services/pre|post/` from prior **DS-001** / **DS-002** sessions removed when executing the hub **`spaghetti-clean-task` / `spaghetti-reset-task`** recipe (recovery from **Git** or **CI** if needed).

---

## [0.6.1] - 2026-04-12

**Summary:** Everything merged on the default branch **after** the **`v0.6.0`** release point through the **documentation and task housekeeping checkpoint** (`b031405`), the **structure-review hub** task files (reset-then-check, scan gates, empty template), and the **despaghettification input-list closure** for **DS-002 through DS-005** (packaging payloads, narrative commit path modules, AI stack report section builders, thin user PUT and validated-turn pipeline companions, plus governance pre/post artefacts and changelog capture for this patch). **Follow-up (2026-04-10):** further **despaghettification** slices **DS-022**, **DS-024**–**DS-026**, and **DS-025** — an early God of Carnage scene-candidate helper that was later superseded by semantic move contracts, **analytics** content queries, LangGraph **dramatic review** packaging, **capability** invocation audit summaries, plus a **spaghetti AST scan** repair and optional **workstream artefact tree** reset per hub clean task.

### Added

- **God of Carnage scene-candidate helper (hub wave DS-022, superseded):** this
  historical entry referred to a keyword-based helper that is no longer part of
  the runtime. Current GoC scene direction requires semantic move payloads and
  uses a neutral `semantic_move_required` fallback when they are missing.
- **Analytics content aggregation (hub wave DS-024):** [`backend/app/services/analytics_service_content.py`](backend/app/services/analytics_service_content.py) — `_query_popular_tags`, `_query_trending_threads`, `_query_content_freshness`, and `build_analytics_content_payload`; [`backend/app/services/analytics_service.py`](backend/app/services/analytics_service.py) keeps a thin `get_analytics_content` that parses the date window and delegates.
- **LangGraph dramatic review section (hub wave DS-026):** [`ai_stack/langgraph/langgraph_runtime_package_output_dramatic_review.py`](ai_stack/langgraph/langgraph_runtime_package_output_dramatic_review.py) — helper functions plus `build_dramatic_review_section`; [`ai_stack/langgraph/langgraph_runtime_package_output_sections.py`](ai_stack/langgraph/langgraph_runtime_package_output_sections.py) re-exports for [`langgraph_runtime_package_output.py`](ai_stack/langgraph/langgraph_runtime_package_output.py).
- **Capability invocation audit summaries (hub wave DS-025):** [`ai_stack/capabilities/capabilities_invocation_summaries.py`](ai_stack/capabilities/capabilities_invocation_summaries.py) — per `wos.*` handlers and `summarize_invocation_result`; [`ai_stack/capabilities/capabilities.py`](ai_stack/capabilities/capabilities.py) aliases `_summarize_invocation_result` (lazy import of `build_retrieval_trace` for the context-pack branch to avoid a load-time import cycle).
...
- **Narrative threads commit path (DS-003):** `narrative_threads_commit_path_utils.py` and `narrative_threads_update_from_commit_phases.py` with a frozen **`NarrativeCommitThreadDrive`**; thin `update_narrative_threads_from_commit_impl` orchestration in `narrative_threads_update_from_commit.py`.
- **AI stack governance report assembly (DS-004):** `ai_stack_closure_cockpit_report_sections.py` and `ai_stack_evidence_session_bundle_sections.py` as section builders; slim entrypoints in `ai_stack_closure_cockpit_report_assembly.py` and `ai_stack_evidence_session_bundle.py`.
- **Writers Room packaging payloads (DS-002):** `writers_room_pipeline_packaging_payloads.py` plus early/tail phase helpers co-located with `run_writers_room_packaging_stage` in `writers_room_pipeline_packaging_stage.py` (same **`WritersRoomPackagingStageResult`** contract).
- **Validated turn pipeline companions (DS-005 optional):** `turn_executor_validated_pipeline_apply.py` and `turn_executor_validated_pipeline_narrative_log.py`; **`user_put_collect_service_kwargs`** in `user_routes_users_update_guards.py` for a thin **`execute_users_update_put`**.
- **Governance artefacts** for the waves above under `despaghettify/state/artifacts/workstreams/backend_runtime_services/pre|post/` (session files for DS-002, DS-003, DS-004, DS-005 optional thin slice) with matching updates to **`despaghettification_implementation_input.md`** and **`WORKSTREAM_BACKEND_RUNTIME_AND_SERVICES_STATE.md`**.
...
- **Writers’ room packaging and inspector assembly (structure list item six):** the long packaging path is split into **sub-stages** (issue extraction, recommendation bundling, and **DS-002** payload builders + phased orchestration); inspector “fill this section” logic moves into helper modules under `backend/app/services/`.
- **Runtime narrative pipeline (same list item seven):** decision guard checks are extracted; orchestration code lines up with the new narrative state objects; **commit-path** packaging for thread updates is delegated to dedicated modules (**DS-003**).
- **Runtime validated-turn pipeline:** `run_validated_turn_pipeline` composes apply and narrative logging modules; **`tools/ds005_runtime_import_check.py`** frozen import list extended for the new modules; **`backend/app/runtime/package_classification.py`** lists the new runtime roots.
- **User PUT `/users/<id>`:** `execute_users_update_put` delegates auth, password rejection, and service kwargs to **`user_routes_users_update_guards`** helpers (**DS-005** optional thin slice).
- **Structure hub documentation:** the living **implementation input** list marks **DS-002–DS-005** closed for these slices; **recommended implementation order** phases 2–5 marked done; **latest structure scan** “as of” and **open hotspots** text refreshed (full **M7** / leaderboard numbers still deferred to a future **`spaghetti_ast_scan`** pass unless the check-task trigger fires).
- **Hub task files:** `spaghetti-reset-task.md` spells out “clean temp files and template first, then run one structure scan pass”; `spaghetti-check-task.md` carries the seven-category score gates, mandatory “as of” clock on every scan, and no longer mentions the reset recipe; `templates/despaghettification_implementation_input.EMPTY.md` and the live scan table match those rules.
- **Despaghettify hub — workstream session trees:** [`spaghetti-clean-task.md`]('fy'-suites/despaghettify/spaghetti-clean-task.md) procedure removes all files under `despaghettify/state/artifacts/workstreams/<slug>/pre|post/` for every slug in [`WORKSTREAM_INDEX.md`]('fy'-suites/despaghettify/state/WORKSTREAM_INDEX.md) and recreates empty folders; optional ephemeral cleanup (caches, local `var/` trees, scratch under `despaghettify/` per the same doc). **Local-only** governance evidence — keep **Git**, **CI**, or **PR** history if you need prior session filenames.

### Fixed

- **Spaghetti AST scan (`collect_ast_stats`):** [`despaghettify/tools/spaghetti_ast_scan.py`]('fy'-suites/despaghettify/tools/spaghetti_ast_scan.py) now unpacks all five fields returned by `metrics()` (including the magic-literal count) when building **top-12** longest and **top-6** nesting summaries — fixes `ValueError: too many values to unpack` after `metrics()` began emitting five-tuple rows.
...
- **Turn executor system-error regression test:** monkeypatch targets **`turn_executor_validated_pipeline_apply.validate_decision`** after the pipeline split so the failure-path test still instruments the correct module.
...
- **Structure clean-up (behaviour preserved):** large **Python** runtime and **artificial-intelligence** stack files were split into smaller modules with clearer import edges while outward behaviour stayed the same.
  ...
  - `ai_stack/capabilities/capabilities.py` — default capability registry moved to a dedicated module with lazy attribute access so imports stay stable.
...
- **`ai_stack/mcp/mcp_static_catalog.py`**: `MCP_RESOURCE_SPECS` / `MCP_PROMPT_SPECS`, `mcp_exposure_counts_by_suite`, `mcp_suite_registry_rows` (single source for cockpit + MCP server listings).
...
- **`tools/mcp_server/resource_prompt_support.py`**: imports resource/prompt specs from `ai_stack.mcp.mcp_static_catalog`.
...
- **`ai_stack/mcp/wos_vsl_mcp_metrics.py`**: Static helpers for write-capable / high-risk mutation counts and read-via-resource share (roadmap §10.3).
...
- **`ai_stack/mcp/mcp_canonical_surface.py`**: `McpSuite`, `mcp_suite` on every canonical tool descriptor, `resolve_active_mcp_suite_filter`, `canonical_tool_names_for_suite`; `wos.session.logs` and `wos.session.state` reclassified as `read_only`; public metadata includes `mcp_suite`.
...
- **Documentation layout**: former `docs/architecture/*` canon and closure material moved under `docs/archive/architecture-legacy/`; active technical docs under `docs/technical/`; RAG task and superpowers execution artifacts archived under `docs/archive/*`; index and cross-links updated across `docs/testing-setup.md`, routing governance closure reports, `llm-slm-role-stratification.md`, `ai_story_contract.md`, and related audit baselines.
...
- Obsolete or superseded backend test modules (e.g. legacy routing governance workstream split files, duplicated drift/narrative task filenames, broad `test_coverage_expansion` / `test_session_api_closure` in favor of focused suites) as reflected in the current tree.
...
- **Capability/MCP audit parity**
  ...
  - `ai_stack/mcp/mcp_canonical_surface.py`, `tools/mcp_server/tools_registry.py`, and `ai_stack/__init__.py` updated so the full research/canon tool and contract surface is exported and reachable through canonical MCP/capability paths.
...
- **`scripts/install-ai-stack-test-env.sh`**, **`scripts/install-ai-stack-test-env.ps1`**, **`scripts/install-ai-stack-test-env.bat`**: mirror `.github/workflows/ai-stack-tests.yml` (editable `story_runtime_core` + `ai_stack[test]`) and verify `langchain_core` / `langgraph` / `ai_stack.langgraph.langgraph_runtime`.
...
- **routing governance**: Importable **`ROUTING_AUTHORITY_REGISTRY`**, **`operator_truth`** on operator audit (Runtime, Writers-Room, Improvement), **`test_convergence_gates`** (**G-CONV-01..08**) (`7cf182e`). **Startup profiles**, **`legibility` / `route_status`**, **`canonical_authority_summary`**, registry entries, **`test_final_closure_gates`** (**G-FINAL-01..08**) (`0452dfd`). **Repository testability**: **`backend/requirements-test.txt`**, smoke runners, **`docs/testing-setup.md`**, **`docs/test-environment-hygiene.md`** (`4ff5490`). Mandatory **setup scripts** and critical install documentation (`c051e6b`, `5eb8387`). **Task 2** registry/routing convergence closure (`814e217`). **Writers-Room / improvement** tests toward **85%+** module coverage; suite registration/reorg (`ffb22e7`, `612b9da`, `95ae7e9`, `bce3d66`, `784ce97`). **Dual workstream** closure (**G-A**, **G-B**), canonical validation command helpers, closure reports (`7cf0d27`). Test-runner note on **per-suite coverage** semantics (`5d9d3c2`). **`compact_operator_comparison`** (**grammar `operator_comparison_v1`**) and **G-T3** gates (`1e72fce`). **Requirements-test hygiene** static checks, CI job, gate enforcement (`fcbe15c`). **Task 4** validation gates (**G-T4-01..08**), E2E three-surface proof, subprocess stability, docs (`4db1dd6`).
- **MCP**: **M1** single canonical descriptor strand (**`ai_stack/mcp/mcp_canonical_surface.py`**), registry **`tool_class`** + **`WOS_MCP_OPERATING_PROFILE`**, enriched **`wos.capabilities.catalog`**, **`wos.mcp.operator_truth`**, audit fields on tool calls; docs **`docs/mcp/12_M1_canonical_parity.md`** (`43cfd25`). M1 **closure** report + gate-complete evidence (`a00d9c0`). M1 **repair**: lightweight import path vs optional heavy deps, **G-MCP-08** report singularity, deferred session tool **input schemas**, canonical vs legacy report superseding (`3d911d7`). **M2** deep operational parity: descriptor derivation helpers, **`wos.session.get` / `wos.session.diag`** implemented, **`ai_stack/pyproject.toml`**, MCP server test deps (`93974c4`). **`.mcp.json`** at repo root (`ac9c577`). **Session tools** **`wos.session.logs` / `state` / `execute_turn`** as full handlers; canonical descriptors **implemented**; operator-truth condensation; per-tree **`pyproject.toml`** files (frontend, administration-tool, world-engine) with optional test extras (`9e91e51`). **`backend/pyproject.toml`** and **`story_runtime_core/pyproject.toml`** expanded (`a2eeca3`, `201afc8`).
...
- New / extended suites under **`backend/tests/runtime/`** (routing governance gates, convergence, final closure, task 3/4), **`tests/smoke/`**, **`tests/requirements_hygiene/`**, **`ai_stack/tests/`**, **`tools/mcp_server/tests/`**, **`story_runtime_core/tests/`**, dedicated **`tests/writers_room/`** and **`tests/improvement/`** layouts, plus architecture-linked gate reports in **`docs/architecture/`** and **`docs/reports/`**.

### Docs

- routing governance gate and closure markdown (**`*_closure*.md`**, validation hardening), **`docs/testing-setup.md`**, test-environment hygiene, MCP M1 parity doc, RAG task notes, stratification / AI story contract updates as referenced in the commits above.
...
- **M7 — LangGraph** (`ai_stack/langgraph/langgraph_runtime.py`): Runtime turn graph with explicit state/nodes, fallbacks, **`graph_*` diagnostics**; workflow seeds for Writers-Room and improvement.
- **M8 — MCP / capabilities** (`ai_stack/capabilities/capabilities.py`): Guarded capability registry (schemas, mode gating, audit, denial); **`wos.context_pack.build`** on runtime path; **`GET /api/v1/sessions/<session_id>/capability-audit`**; **`wos.capabilities.catalog`** alignment.
...
- **Tests:** `test_areas_api.py` (areas CRUD, user areas GET/PUT, feature-areas list/put, auth/me allowed_features); conftest calls `ensure_areas_seeded()`; `test_home_returns_200` fixed for current landing content (WORLD OF SHADOWS / Better Tomorrow).
...
- **Startup mode log:** On backend startup, a single line is always logged indicating the current mode: `Running Better Tomorrow Backend [mode: TESTING]`, `[mode: NORMAL (MAIL_ENABLED=1)]`, or `[mode: DEV (MAIL_ENABLED=0)]` (`app/__init__.py`).
...
- **Seed/example news:** `flask seed-news` (requires `DEV_SECRETS_OK=1`) creates a small set of example news entries for development and validation. Themes: project announcement, backend/frontend split (development), news system live (features), World of Better Tomorrow (lore), API and CORS setup (technical), and one draft (Upcoming Events). Categories: Announcements, Development, Features, Lore, Technical. Five published and one draft so list/detail, search, sort, and category filter can be tested. Author is set from the first user if any. Skips slugs that already exist. Data is loaded by running the CLI once after `flask init-db` (and optionally `flask seed-dev-user`).
