...
- **Contractify hub tools:** `python -m pytest "'fy'-suites/contractify/tools/tests" -q` (from repo root, after **`pip install -e .`**) — twenty-three tests; local audit JSON under **`'fy'-suites/contractify/reports/*.json`** is gitignored to avoid committing machine-generated drift snapshots.
