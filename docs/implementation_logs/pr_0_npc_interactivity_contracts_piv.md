...
| `god_of_carnage_scene_director.py:911-943` for `_build_responder_set()` | `ai_stack/story_runtime/director/god_of_carnage_scene_director.py:655` (def) and `:911` (call site) | Plan's range was the call site; the function definition is much earlier. |
...
- `ai_stack/story_runtime/runtime_aspect_ledger/__init__.py` — no change. `ASPECT_KEYS` is unchanged; no Director-Pause aspect row is added.
