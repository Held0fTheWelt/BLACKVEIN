...
**Task 2B / Task 2C / Task 2E integration (current):**

- **Runtime**: The canonical AI turn path still evaluates routing **once** per turn (`route_model` before a single adapter execution). Task 2E adds **internal** staged policy inside that single call (exclusions → mandatory escalation constraints → role-family → ranking → cost/latency → widening/fallback); it does **not** add chained SLM→LLM Runtime orchestration. When no spec-backed adapter is eligible, execution falls back to the adapter already supplied by the caller; guards and commit semantics are unchanged. `AIDecisionLog` carries `model_routing_trace` (request + decision + execution metadata). **Task 2C-2** adds nested **`routing_evidence`** (phase, task kind, selected vs executed adapters, `route_reason_code`, `fallback_chain`, flags, `no_eligible_spec_selection`, runtime fallback fields). **Task 2D** adds `policy_execution_aligned` and `execution_deviation`. **Task 2E** adds granular primary `RouteReasonCode` values for escalation, richer honest `decision_factors`, and a deterministic **`routing_overview`** summary derived from the emitted primary reason (not independent model reasoning). Values reflect what the code actually supplies — not invented granularity. **Task 2E-R1** narrows `escalation_due_to_structured_output_gap` so it applies only when structured-output eligibility **materially changes** the deterministic primary versus the full pre-structured candidate set — not merely when the global eligible list shrinks; the broader staged 2E architecture stays the same, and deep multi-stage Runtime orchestration remains out of scope. **Task 2F** adds **`diagnostics_overview`** (fixed-vocabulary `summary`, `severity`, allowlisted **`operator_hint`**, **`short_explanation`**), **`diagnostics_flags`**, and ordered **`diagnostics_causes`** on the same `routing_evidence` object — readability and operator visibility only; **`routing_overview`** remains for backward compatibility. The compact layer is derived strictly from existing evidence fields and does not change routing semantics or Runtime orchestration.
- **Writers Room**: Same contracts via `route_model` with **preflight** and **synthesis** stages. `model_generation.task_2a_routing` holds per-stage traces; each stage includes **`routing_evidence`** in the same normalized shape as runtime (plus bounded-call / skip fields where applicable).
- **Improvement**: After deterministic sandbox evaluation and recommendation thresholds, the Improvement experiment HTTP path runs **two** bounded routing stages (preflight + synthesis) using the same spec source as Writers Room. The package exposes `task_2a_routing`, `deterministic_recommendation_base`, and `model_assisted_interpretation` (advisory only). **Model output does not replace** threshold-based recommendation truth; **authoritative truth** stays with deterministic evaluation and engine rules.

**Not in scope:** separate telemetry pipelines, dashboards, autonomous multi-turn editorial stacks, or **deep multi-stage Runtime orchestration** (the canonical turn still **routes once** and executes **one** adapter; no SLM→LLM narrative pipeline inside that path).
