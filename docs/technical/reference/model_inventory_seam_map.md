...
**routing governance authority classification (machine-readable):** each component is classified exactly once in [`backend/app/runtime/routing/routing_authority.py`](../../backend/app/runtime/routing/routing_authority.py) (`ROUTING_AUTHORITY_REGISTRY`). Canonical Runtime / Writers-Room / Improvement Task 2A paths do **not** treat `ai_stack` LangGraph `RoutingPolicy` as authoritative; that stack is **compatibility-only** relative to `route_model`.
...
- Conflation with [`role_contract.py`](../../backend/app/runtime/ai/role_contract.py) (intra-call shape, not cross-model routing).
