...
**routing governance authority classification (machine-readable):** each component is classified exactly once in [`backend/app/runtime/routing_authority.py`](../../backend/app/runtime/routing_authority.py) (`ROUTING_AUTHORITY_REGISTRY`). Canonical Runtime / Writers-Room / Improvement Task 2A paths do **not** treat `ai_stack` LangGraph `RoutingPolicy` as authoritative; that stack is **compatibility-only** relative to `route_model`.
