...
- **Callback web:** derives bounded `callback_web_record.v1` evidence from committed history, narrative threads, continuity classes, scene anchors, and branch timeline events. It persists through `JsonCallbackWebStore`, exposes `callback_web_summary` and `callback_web_feedback.v1`, feeds bounded prior callback context into the next graph turn, and records the `callback_web` runtime aspect. It is diagnostics and context support only; it does not mutate canon or adopt simulated branch state. See [`callback_web_contract.md`](callback_web_contract.md).
- **Consequence cascade:** derives bounded `consequence_cascade_record.v1` evidence from committed history, narrative threads, continuity classes, resolved pressures, and branch timeline selections. It persists through `JsonConsequenceCascadeStore`, exposes `consequence_cascade_summary` and `consequence_cascade_feedback.v1`, feeds bounded prior cascade context into the next graph turn, and records the `consequence_cascade` runtime aspect. It is committed-state feedback only; it does not mutate canon, adopt simulated branch state, or use generated prose as the oracle. See [`consequence_cascade_contract.md`](consequence_cascade_contract.md).
