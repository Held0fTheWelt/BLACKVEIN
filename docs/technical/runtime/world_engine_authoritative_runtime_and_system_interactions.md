...
Each story session is a small, server-side record: which module, which scene you are in, committed environment state, how many turns ran, a bounded history of committed turns plus recoverable audit turns, a policy-driven hierarchical memory snapshot derived only from committed turns, and runtime-aspect evidence for what the engine allowed the model to reveal or imply.
...
- **Callback web:** derives bounded `callback_web_record.v1` evidence from committed-truth history, narrative threads, continuity classes, scene anchors, and branch timeline events. Recoverable/false-commit rows are retained for audit/story-window continuity but filtered before callback observations and edges are built. It persists through `JsonCallbackWebStore`, exposes `callback_web_summary` and `callback_web_feedback.v1`, feeds bounded prior callback context into the next graph turn, and records the `callback_web` runtime aspect. It is diagnostics and context support only; it does not mutate canon or adopt simulated branch state. See [`callback_web_contract.md`](callback_web_contract.md).
- **Consequence cascade:** derives bounded `consequence_cascade_record.v1` evidence from committed-truth history, narrative threads, continuity classes, resolved pressures, and branch timeline selections. Recoverable/false-commit rows are retained for audit/story-window continuity but filtered before consequence atoms and edges are built. It persists through `JsonConsequenceCascadeStore`, exposes `consequence_cascade_summary` and `consequence_cascade_feedback.v1`, feeds bounded prior cascade context into the next graph turn, and records the `consequence_cascade` runtime aspect. It is committed-state feedback only; it does not mutate canon, adopt simulated branch state, or use generated prose as the oracle. See [`consequence_cascade_contract.md`](consequence_cascade_contract.md).
- **Pacing rhythm:** derives bounded `pacing_rhythm_state` and `pacing_rhythm_target` from scene energy, pacing/silence decisions, prior rhythm, narrative-thread pressure, and callback-web feedback. Validation checks structured visible-block and actor-turn counts, not prose rhythm. The result is recorded as `turn_aspect_ledger.pacing_rhythm`, persisted into planner truth, rehydrated into the next turn, and exposed through Langfuse/MCP. See [`pacing_rhythm_contract.md`](pacing_rhythm_contract.md).
- **Temporal control:** derives bounded `temporal_control_state` and `temporal_control_target` from scene plan, scene energy, pacing, semantic move, callback-web, and consequence-cascade feedback. Validation checks structured `temporal_control_events` against selected operations, committed source refs, elapsed-turn bounds, history-rewrite flags, and branch-state adoption flags. It records `turn_aspect_ledger.temporal_control`, persists planner-truth feedback, rehydrates `prior_temporal_control_state`, and exposes Langfuse/MCP fields. See [`temporal_control_contract.md`](temporal_control_contract.md).
- **Social pressure:** derives bounded continuous `social_pressure_state` and `social_pressure_target` from `scene_assessment`, `social_state_record`, scene energy, pacing rhythm, prior planner truth, and committed narrative-thread pressure. Validation checks schema and policy-threshold consistency, not prose intensity. The result is recorded as `turn_aspect_ledger.social_pressure`, persisted into planner truth, rehydrated into the next turn, and exposed through Langfuse/MCP. See [`social_pressure_contract.md`](social_pressure_contract.md).
- **Relationship state:** derives bounded durable `relationship_state_record` and `relationship_dynamics_target` from canonical relationships, structured social state, social pressure, NPC initiative-pressure edges, continuity evidence, and prior committed relationship state. Validation checks structured `relationship_dynamics_events` for selected target ids, transition codes, schema bounds, and actor-lane compatibility, not prose chemistry. The result is recorded as `turn_aspect_ledger.relationship_state`, persisted into planner truth, rehydrated into the next turn, and exposed through Langfuse/MCP. See [`relationship_state_machine_contract.md`](relationship_state_machine_contract.md).
- **Genre awareness:** derives bounded `genre_awareness_state` and `genre_awareness_target` from module genre policy, scene-plan context, adjacent runtime targets, and prior planner truth. Validation checks structured `genre_awareness_events` for selected profile id, allowed register, required conventions, forbidden markers, event presence, and signal budget, not generated genre prose. The result is recorded as `turn_aspect_ledger.genre_awareness`, persisted into planner truth and governance payloads, rehydrated into the next turn, and exposed through Langfuse/MCP. See [`genre_awareness_contract.md`](genre_awareness_contract.md).
- **Sensory context:** derives bounded `sensory_context_state` and `sensory_context_target` from module sensory policy, authored narrator sensory palette, scene affordances, current location/object focus, scene energy, pacing rhythm, social pressure, prior planner truth, and output language. Validation checks structured `sensory_context_events` for selected layer ids, required layers, source references, and layer budget, not prose vividness. The result is recorded as `turn_aspect_ledger.sensory_context`, persisted into planner truth, rehydrated into the next turn, and exposed through Langfuse/MCP. See [`sensory_context_contract.md`](sensory_context_contract.md).
- **Improvisational coherence:** derives a bounded `improvisational_coherence_target` from interpreted input, semantic move records, scene-plan fields, selected responders, scene energy, pacing rhythm, and module policy. Validation checks structured `improvisational_coherence_events` for contribution acknowledgement, allowed mode/class, scene anchors, truth safety, forced-player-revision flags, and playable boundary reasons. The result is recorded as `turn_aspect_ledger.improvisational_coherence` and exposed through Langfuse/MCP; it is generation governance and retry feedback, not canon mutation. See [`improvisational_coherence_contract.md`](improvisational_coherence_contract.md).
- **Subtext interpretation:** builds bounded `SemanticMoveRecord.subtext` records from `content/modules/god_of_carnage/direction/subtext_policy.yaml`. The runtime records surface mode, hidden-intent hypothesis, subtext function, sincerity band, evidence codes, and policy provenance. This may shape director pressure, pacing, packet context, and diagnostics, but it does not create committed facts or expose hidden NPC truth. Path summaries, Langfuse spans/scores, inspector projections, and operator history surface the same Pi19 fields.
...
- **Expectation variation:** loads module `runtime_intelligence.expectation_variation` through `ModuleRuntimePolicy`, derives a bounded `expectation_variation` target from selected setup evidence, prior cooldown state, and adjacent runtime aspects, and validates structured `expectation_variation_events` against selected ids, variation types, setup refs, and per-turn budget. This is the implemented Pi29 surprise-budget slice, but the oracle is policy/schema/ledger evidence, not generated surprise prose.
