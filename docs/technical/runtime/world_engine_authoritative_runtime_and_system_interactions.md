...
- **Recoverable playability:** recoverable validation rejections and expected graph `RuntimeError` failures can return player-visible `rejected_recoverable` turns with `recoverable_playability.v1`, `commits_story_truth=false`, and an explicit retry/redirect affordance. These turns are audit/story-window records, not committed truth for memory, callback, or cascade feedback.
