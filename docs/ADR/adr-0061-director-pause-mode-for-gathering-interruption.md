...
Pure function in the Director (the canonical surface will live in `ai_stack/scene_director_goc.py`; the function is introduced by PR-C, not PR-0).
...
- The NPC-agency / mandatory-beat-consumption path (canonical surface around `ai_stack/scene_director_goc.py:655` `_build_responder_set()` and `ai_stack/langgraph_runtime_executor.py:3996` `_build_npc_agency_plan_projection()`) is consulted with a `gathering_paused` gate. The gate **does not** suppress NPC mundane action; it only suppresses mandatory-beat consumption tied to the missing co-presence.
