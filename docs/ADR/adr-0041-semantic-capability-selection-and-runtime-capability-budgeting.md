...
| Runtime intelligence projection hook | Implemented | `runtime_intelligence_projection.capability_selection` is derived locally from existing turn context; it does not mark the ledger capability aspect as passed and does not affect commit/readiness gates. Implementation lives in `ai_stack/story_runtime/runtime_aspect_ledger/runtime_intelligence_projection/`: source collectors read canonical records, section builders assemble named diagnostic surfaces, `semantic_dispatch.py` builds capability/validator projections, and `adr_sidecar_projection.py` attaches optional ADR-0041 graph sidecars. |
...
The runtime-intelligence projection is assembled by the
`ai_stack/story_runtime/runtime_aspect_ledger/runtime_intelligence_projection/`
package. The package is intentionally split into record-source collectors,
field catalogs, section builders, semantic dispatch construction, and ADR-0041
sidecar attachment so that each projection surface is named by its runtime
responsibility. It remains a derived diagnostic view; canonical commit and
validation truth stay with `run_validation_seam` and the normalized aspect
records.
