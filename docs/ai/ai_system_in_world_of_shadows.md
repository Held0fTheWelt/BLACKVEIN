...
**Not:** LangGraph does not replace `StoryRuntimeManager`; the host still owns `turn_counter`, `history`, `diagnostics`, `resolve_narrative_commit`, and narrative threads. The Meta/OOC branch is not `meta_narrative_awareness`; it is a non-story control path with structured diagnostics.
...
**Technical:** `ContextSynthesisBundle` is defined in [`ai_stack/contracts/context_synthesis_contracts.py`](../../ai_stack/contracts/context_synthesis_contracts.py) and built by [`ai_stack/story_runtime/narrative/context_synthesis_engine.py`](../../ai_stack/story_runtime/narrative/context_synthesis_engine.py). The LangGraph runtime inserts `synthesize_context` between `director_select_dramatic_parameters` and `assemble_model_context`, then exposes the bounded summary at `graph_diagnostics.context_synthesis`. Self-correction retries call the same deterministic engine with `validation_feedback`; diagnostics retain `context_synthesis_retry_history`, `resynthesis_count`, and `used_for_self_correction`.
