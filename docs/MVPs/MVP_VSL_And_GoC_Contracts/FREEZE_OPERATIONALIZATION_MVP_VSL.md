...
| Diagnostics / logging surfaces | `package_output` sets `graph_diagnostics`: graph name/version, nodes executed, node outcomes, fallback path taken, execution health, errors, capability audit, repro metadata, operational cost hints. | partial | `ai_stack/langgraph/langgraph_runtime.py` (`_package_output`) | Strong starting point for operator-facing diagnostics, but not yet the full dramatic replay surface required by the target slice. |
