...
Contractual frame for the MVP vertical slice **God of Carnage (GoC)** under Phase 0 Freeze. Normative process rules live in `docs/MVPs/MVP_VSL_And_GoC_Contracts/FREEZE_OPERATIONALIZATION_MVP_VSL.md`; product intent in `docs/MVPs/MVP_VSL_And_GoC_Contracts/ROADMAP_MVP_VSL.md`. Other freeze artifacts may reference this document; they must not redefine the same concepts in conflicting terms.
...
For input or situations outside the GoC slice: prefer **containing, scene-coherent** handling (Roadmap §9); do not extend canonical truth beyond the slice. Concrete visible and diagnostic responses are anchored in `docs/MVPs/MVP_VSL_And_GoC_Contracts/GATE_SCORING_POLICY_GOC.md` (Failure-to-Response).
...
|---|---|---|---|---|
| Runtime graph / orchestration path | `StateGraph(RuntimeTurnState)` starts at `translate_player_input` → `interpret_input`, then branches: `player_input_kind=meta` → `meta_control_turn` → `package_output` → `END`; story-play input → `resolve_player_action` → `director_compose_realization` → `realize_via_capabilities` → `route_model` → `invoke_model` (conditional `fallback_model`) → `proposal_normalize` → `validate_seam` → `commit_seam` → `render_visible` → `package_output` → `END` (ADR-0062 thin path). LDSS nodes (`retrieve_context`, `goc_resolve_canonical_content`, scene director, `derive_*`, `synthesize_context`, `assemble_model_context`) remain in graph source but are not on this default player edge list. Removed: `authoritative_action_resolution`. | implemented (GoC slice, PR-A 2026-05-19) | `ai_stack/langgraph/langgraph_runtime.py` public surface; `ai_stack/langgraph/langgraph_runtime_executor.py` (`RuntimeTurnGraphExecutor._build_graph`) | Further roadmap phases may add nodes; must stay one graph truth surface. Meta/OOC control input is explicitly non-story: no retrieval/model story invocation, no `validate_seam`, and no `commit_seam`. Opt-in `meta_narrative_awareness` is a separate story-play aspect, not the control branch; v2 can use bounded relationship/memory signals for adaptive fourth-wall and selected-memory-ref awareness when fully opted in. |
| Runtime state structures | `RuntimeTurnState` includes transport fields plus GoC canonical groups: `goc_slice_active`, `goc_canonical_yaml`, `goc_yaml_slice`, `interpreted_move`, `scene_assessment`, `selected_responder_set`, `selected_scene_function`, `pacing_mode`, `silence_brevity_decision`, `proposed_state_effects`, `validation_outcome`, `committed_result`, `visible_output_bundle`, `continuity_impacts`, `visibility_class_markers`, `failure_markers`, `fallback_markers`, `diagnostics_refs`, `experiment_preview`, `transition_pattern`, plus `graph_diagnostics`, `nodes_executed`, `node_outcomes`, etc. | implemented | `ai_stack/langgraph/langgraph_runtime.py` (`RuntimeTurnState`) | Map to operator projection via `build_operator_canonical_turn_record` (`ai_stack/story_runtime/turn/god_of_carnage_turn_seams.py`). |
| Turn-related contracts / vocabulary | `AdapterInvocationMode` / `ExecutionHealth` in `runtime_turn_contracts`; dramatic vocabulary on state (`selected_scene_function`, pacing, silence, continuity classes, visibility classes) and in `graph_diagnostics.dramatic_review`; Meta/OOC control path markers (`adapter_invocation_mode=meta_control_path`, `graph_path_summary=meta_control_deterministic`, `commit_not_applicable=true`). | implemented (GoC) | `ai_stack/contracts/runtime_turn_contracts.py`; `ai_stack/langgraph/langgraph_runtime.py`; `ai_stack/langgraph/langgraph_runtime_executor.py` | Vocabulary changes remain freeze-amendment only (`VERTICAL_SLICE_CONTRACT_GOC.md` §5). |
...
| Mock/fallback/test infrastructure (dry-run) | Primary path uses LangChain/runtime adapter invocation in `_invoke_model`. Fallback path uses `adapters.get("mock")` in `_fallback_model`; metadata marks managed fallback or degraded missing-fallback mode. | partial | `ai_stack/langgraph/langgraph_runtime.py` (`_invoke_model`, `_fallback_model`) | Dry-run should build on this existing path rather than inventing an abstract separate concept. |
| Diagnostics / logging surfaces | `package_output` sets `graph_diagnostics` (including `dramatic_review`), `diagnostics_refs` (graph projection + `experiment_preview` + `transition_pattern` + gate hints), and `repro_metadata` / `repro_complete`. | implemented | `ai_stack/langgraph/langgraph_runtime.py` (`_package_output`); `ai_stack/story_runtime/turn/god_of_carnage_turn_seams.py` (`build_diagnostics_refs`, `build_operator_canonical_turn_record`) | Operator compact replay: canonical turn projection + `graph_diagnostics` (see `GATE_SCORING_POLICY_GOC.md` §5). |
...
| Scene assessment | `director_assess_scene` writes `scene_assessment` from canonical YAML + optional guidance hints (`ai_stack/story_runtime/director/god_of_carnage_scene_director.py`). | Schema-first `scene_assessment`. | implemented | Minimal required keys documented in `CANONICAL_TURN_CONTRACT_GOC.md` §5.1. |
...
- `multi_pressure` — Multiple simultaneous pressure lines; **single `selected_scene_function` priority rule** in `docs/MVPs/MVP_VSL_And_GoC_Contracts/CANONICAL_TURN_CONTRACT_GOC.md` §3.3 applies.
...
**Cross-reference:** Selection of a single `selected_scene_function` when multiple intentions compete — `docs/MVPs/MVP_VSL_And_GoC_Contracts/CANONICAL_TURN_CONTRACT_GOC.md` §3.3.
...
| Aspect | Binding |
|---|---|
| **Infrastructure used** | Existing graph in `ai_stack/langgraph/langgraph_runtime.py`; primary `_invoke_model`; fallback/mock `_fallback_model` with `adapters.get("mock")`; output includes `graph_diagnostics` from `_package_output`. |
...
| **Inspection** | Evaluate `graph_diagnostics` (graph name/version, `nodes_executed`, `node_outcomes`, `fallback_path_taken`, `execution_health`, `errors`, `repro_metadata`, `operational_cost_hints`) plus optional tabletop overlay per `docs/MVPs/MVP_VSL_And_GoC_Contracts/GATE_SCORING_POLICY_GOC.md`. |

---

## 8. Cross-references

- Canonical turn schema, field ownership, seams, scene direction, transitions: **`docs/MVPs/MVP_VSL_And_GoC_Contracts/CANONICAL_TURN_CONTRACT_GOC.md`**
- Gate families, scoring, cadence, diagnostic modes, failure mapping, escalation, tabletop evidence: **`docs/MVPs/MVP_VSL_And_GoC_Contracts/GATE_SCORING_POLICY_GOC.md`**

---

## 9. Tabletop / traceability (pointer)

Full tabletop requirements and evidence structure: **`docs/MVPs/MVP_VSL_And_GoC_Contracts/GATE_SCORING_POLICY_GOC.md`**, Tabletop Trace-Through. This slice document supplies vocabulary, assets, and the current/target bridge against which scenarios are checked per FREEZE §21.
