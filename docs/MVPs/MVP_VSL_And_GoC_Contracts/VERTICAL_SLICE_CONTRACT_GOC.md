...
| Scene assessment | `director_assess_scene` writes `scene_assessment` from canonical YAML + optional guidance hints (`ai_stack/director/scene_director_goc.py`). | Schema-first `scene_assessment`. | implemented | Minimal required keys documented in `CANONICAL_TURN_CONTRACT_GOC.md` §5.1. |
