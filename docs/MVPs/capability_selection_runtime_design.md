...
(``ai_stack/story_runtime/runtime_aspect_ledger.py``) enables plan-enforced local validator execution when tests pass
...
| ADR-0041 harness | `ai_stack/story_runtime/runtime_aspect_ledger.py::build_adr0041_validator_dispatch_harness_report` | Optional plan-enforced execution in tests | Explicit test call only | Test-scoped | No | No | **dry_run_projection_only** (production) / harness-only execution |
