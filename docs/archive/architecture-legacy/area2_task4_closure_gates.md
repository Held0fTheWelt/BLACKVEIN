...
|--------|----------------|-------|-----------------|
| **G-T4-01** | Canonical **Runtime**, **Writers-Room**, and **Improvement** integration truth for operator audit + routing evidence contracts is exercised. | `backend/tests/runtime/test_runtime_validation_commands_orchestration.py::test_full_validation_runtime_writers_room_improvement_operator_truth` (delegates to cross-surface contract tests). | One or more surfaces lack proven staged/HTTP integration shape or contract keys. |
| **G-T4-02** | Bootstrap-on, test-isolated, and profile-implied states are validated on canonical Area 2 paths. | Delegation to startup, operational, and bootstrap integration tests in `test_full_validation_bootstrap_profiles_and_staged_integration`. | Bootstrap or profile classification is not enforced by tests. |
| **G-T4-03** | Cross-surface **compact_operator_comparison** and shared audit contracts remain consistent and regression-protected. | Delegation to operator-comparison and cross-surface contract tests in `test_full_validation_operator_comparison_and_cross_surface_contracts`. | Compact grammar or cross-surface key contract drifts without test failure. |
| **G-T4-04** | Negative, degraded, and missing-adapter paths are honest (no false success). | Delegation to degraded Runtime paths, Improvement negative routing, and related paths in `test_full_validation_degraded_runtime_and_improvement_honesty`. | Degraded truth is under-proven vs healthy paths or overclaims success. |
| **G-T4-05** | Schema/version and bounded routing-evidence key expectations are guarded. | Delegation to `test_runtime_drift_resistance.py` in `test_full_validation_audit_schema_drift_resistance`. | Silent drift of audit or evidence shapes. |
| **G-T4-06** | Documented validation commands match the **canonical** Task 4 module list and flags from code (exact string). | `test_full_validation_documented_pytest_command_matches_code` reads `docs/testing-setup.md` and compares to `area2_task4_full_closure_pytest_invocation()`. | Docs and `area2_validation_commands.py` diverge. |
| **G-T4-07** | Full **proof** module list (canonical Task 4 list **excluding** the gate orchestrator module) passes under pytest. | `test_full_validation_proof_modules_pass_without_gate_recursion` subprocess from `backend/`. | Any proof module regresses; closure suite not green. |
| **G-T4-08** | Architecture docs reference every **G-T4-0x** gate and the closure report + command surface. | `test_full_validation_docs_reference_task4_gates_and_commands`. | Documentation truth for Task 4 gates is incomplete or inconsistent. |
