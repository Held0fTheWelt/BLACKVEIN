...
- **Python**: **3.14.x only** (`requires-python = ">=3.14,<3.15"`; repo root `.python-version` is `3.14`). **CI, Dev Container, and all Compose images** use **3.14**.
...
GitHub Actions uses **Python 3.14** for backend and `ai_stack` jobs. The **Dev Container** (`.devcontainer/devcontainer.json`) uses the same **3.14** image and runs the **same** install sequence as the setup scripts, then adds `world-engine/requirements-dev.txt`. For the closest guarantee to CI green, use **3.14** (see `.python-version`) or open the repo in the Dev Container.
