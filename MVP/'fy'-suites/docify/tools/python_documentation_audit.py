#!/usr/bin/env python3
"""Docify hub CLI — static audit for Python documentation hygiene across the monorepo.

Scans selected source trees with the ``ast`` module and reports module, class,
and function/method definitions that lack a usable docstring. Intended for
humans and coding agents planning incremental documentation work without
changing runtime behavior.

Repository policy: user-facing narrative may vary by layer, but maintainer
facing code comments and docstrings in committed Python stay English; this tool
emits English messages only.
...
Examples:
    ...
    python "./'fy'-suites/docify/tools/python_documentation_audit.py" --json --out doc_audit.json

    # Include tests and private ``_`` symbols
    python "./'fy'-suites/docify/tools/python_documentation_audit.py" --include-tests --include-private

    # Append Ruff issues when ``ruff`` is on PATH (optional)
    python "./'fy'-suites/docify/tools/python_documentation_audit.py" --with-ruff
...
def _maybe_run_ruff(roots: Sequence[Path]) -> tuple[int, str]:
    """Run ``ruff check`` across *roots* when available; return exit code and output."""
    try:
        proc = subprocess.run(
