#!/usr/bin/env python3
"""Remove bulk-docstring placeholder prose from ``ai_stack`` (excluding ``tests/``).

Rewrites only string literals that are the leading docstring of ``Module``,
``ClassDef``, ``FunctionDef``, or ``AsyncFunctionDef`` nodes. Does **not**
change ``# TODO`` comments or non-docstring string literals.

Run from repository root::

    python "./'fy'-suites/docify/tools/strip_ai_stack_docstring_placeholders.py"
...
def _replace_split_todo_document(text: str) -> str:
    """Handle ``name: TODO: document\\n ``name`` (type...).`` (name repeated on next line)."""
