/**
 * Dashboard: metrics and charts via /api/v1/admin/*, site settings, activity log, profile update.
 */
(function () {
  'use strict';

  var API = '/api/v1';
  var METRICS_API = API + '/admin/metrics';
  var LOGS_API = API + '/admin/logs';
  var LOGS_EXPORT_API = API + '/admin/logs/export';
  var SITE_SETTINGS_API = API + '/site/settings';
      ...
      .then(function (res) {
        if (!res.ok) {
          return res.json().catch(function () { return {}; }).then(function (j) {
            throw new Error((j && j.error) || 'Failed to load metrics');
          });
        }
        return res.json();
      })
  ...
  function getFilters() {
    ...
    return { search: search.trim(), category: category, status: status, from: from, to: to };
  }
  ...
  function exportCsv() {
    ...
    var url = LOGS_EXPORT_API + (params.toString() ? '?' + params.toString() : '');
  ...
  function loadSiteSettings() {
    if (!document.getElementById('site-settings-form')) return;
    fetch(SITE_SETTINGS_API, { credentials: 'same-origin', headers: { Accept: 'application/json' } })
  ...
  function saveUserSettings(e) {
    ...
    var uid = typeof window.__DASHBOARD_USER_ID__ === 'number' ? window.__DASHBOARD_USER_ID__ : null;
    if (uid == null) {
      if (statusEl) statusEl.textContent = 'Not signed in.';
      return;
    }
    var nameEl = document.getElementById('user-settings-name');
    var emailEl = document.getElementById('user-settings-email');
    var payload = {
      ...
      .then(function (res) {
        if (statusEl) {
          if (res.ok) {
            ...
            statusEl.textContent = (res.body && res.body.error) ? res.body.error : 'Save failed.';
          }
        }
      })
      .catch(function () {
