"""routing governance — compact operator truth derived only from existing facts + explicit counts."""

from __future__ import annotations

from typing import Any

from app.runtime.routing.operational_state import (
    OperationalState,
    classify_operational_state,
    pytest_session_active,
    rollup_no_eligible_discipline_for_bounded_traces,
)
from app.runtime.routing.startup_profiles import resolve_startup_profile
from app.runtime.routing.model_inventory_contract import InventorySurface
from app.runtime.routing.model_inventory_report import validate_surface_coverage
from app.runtime.model_routing_contracts import RouteReasonCode
from app.runtime.routing.no_eligible_operator_meaning import (
...
OPERATOR_COMPARISON_GRAMMAR_VERSION = "operator_comparison_v1"

# Mandatory keys on ``operator_truth["compact_operator_comparison"]`` (all three surfaces).
...
# Frozen summary aligned with ROUTING_AUTHORITY_REGISTRY narrative (no runtime probe).
...
def _derive_route_status(
    *,
    ...
    if operational_state is OperationalState.test_isolated:
        return "test_isolated_expected_empty_registry"
    if operational_state is OperationalState.misconfigured:
        return "misconfigured_registry_or_inventory"
    if operational_state is OperationalState.intentionally_degraded:
...
def _build_compact_operator_comparison(
    *,
    ...
    return {
        "grammar_version": OPERATOR_COMPARISON_GRAMMAR_VERSION,
        "surface": surface,
...
def build_operator_truth(
    *,
    ...
    """Assemble additive ``operator_truth`` for operator_audit (English keys only)."""
...
def merge_operator_truth(audit: dict[str, Any], truth: dict[str, Any]) -> None:
    """Attach truth to an existing operator_audit dict in place."""
    audit["operator_truth"] = truth
...
def enrich_operator_audit_with_operator_truth(
    audit: dict[str, Any],
    *,
    surface: str,
    authority_source: str,
    ...
    """Populate ``operator_truth`` on an existing operator_audit dict."""
    truth = build_operator_truth(
        surface=surface,
        authority_source=authority_source,
        bootstrap_enabled=bootstrap_enabled,
        registry_model_spec_count=registry_model_spec_count,
    ...
    merge_operator_truth(audit, truth)
