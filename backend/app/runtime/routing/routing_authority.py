"""routing governance — explicit routing/registry authority classification (convergence closure).
...
``ROUTING_AUTHORITY_REGISTRY`` below is the single importable map of those layers.
...
class AuthorityLayer(str, Enum):
    """Exactly one layer per registered routing governance seam."""
...
# Registry of every routing governance line referenced by architecture/tests. Each appears once.
ROUTING_AUTHORITY_REGISTRY: tuple[RoutingAuthorityEntry, ...] = (
    ...
    RoutingAuthorityEntry(
        ...
        layer=AuthorityLayer.non_authoritative_support,
        module_path="app.runtime.routing.model_inventory_contract",
    ...
    RoutingAuthorityEntry(
        ...
        layer=AuthorityLayer.non_authoritative_support,
        module_path="app.runtime.routing.model_inventory_report",
    ...
    RoutingAuthorityEntry(
        ...
        layer=AuthorityLayer.non_authoritative_support,
        module_path="app.runtime.routing.model_routing_evidence",
    ...
    RoutingAuthorityEntry(
        ...
        layer=AuthorityLayer.non_authoritative_support,
        module_path="app.runtime.routing.operator_audit",
        description="Deterministic operator views from existing traces; no new policy.",
        canonical_for_task2a_paths=frozenset(CanonicalSurface),
    ),
    RoutingAuthorityEntry(
        component_id="operator_truth",
        layer=AuthorityLayer.non_authoritative_support,
        module_path="app.runtime.routing.operator_truth",
        description="Derived operator legibility and cross-surface truth from existing traces; no new telemetry.",
        canonical_for_task2a_paths=frozenset(CanonicalSurface),
    ),
    RoutingAuthorityEntry(
        component_id="startup_profiles",
        layer=AuthorityLayer.non_authoritative_support,
        module_path="app.runtime.routing.startup_profiles",
    ...
    RoutingAuthorityEntry(
        ...
        layer=AuthorityLayer.non_authoritative_support,
        module_path="app.runtime.ai_turn.runtime_ai_stages",
    ...
    RoutingAuthorityEntry(
        ...
        layer=AuthorityLayer.non_authoritative_support,
        module_path="app.runtime.ai_turn.ai_turn_executor",
...
def authority_entries_for_surface(surface: CanonicalSurface) -> tuple[RoutingAuthorityEntry, ...]:
    """Return entries that participate in the given canonical Task 2A surface."""
    return tuple(e for e in ROUTING_AUTHORITY_REGISTRY if surface in e.canonical_for_task2a_paths)
...
def assert_routing_policy_entry_is_unique_authoritative_policy() -> None:
    ...
    policies = [e for e in ROUTING_AUTHORITY_REGISTRY if e.component_id == "task2a_route_model"]
...
def assert_langgraph_not_canonical_for_task2a() -> None:
    ...
    lg = next(e for e in ROUTING_AUTHORITY_REGISTRY if e.component_id == "langgraph_runtime_routing_policy")
