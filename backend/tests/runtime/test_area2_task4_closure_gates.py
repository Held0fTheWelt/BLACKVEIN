"""Area 2 Task 4 — final validation, drift resistance, and E2E truth closure (G-T4-01 … G-T4-08)."""
...
def _load_tests_py_module(relative_under_tests: str):
    ...
    name = path.stem
    spec = importlib.util.spec_from_file_location(f"_area2_t4_{name}", path)
...
@pytest.mark.asyncio
async def test_g_t4_01_end_to_end_truth_three_surfaces_gate(
    client,
    auth_headers,
    minimal_module,
):
...
@pytest.mark.asyncio
async def test_g_t4_02_bootstrap_validation_gate(minimal_module):
...
@pytest.mark.asyncio
async def test_g_t4_03_cross_surface_contract_gate(
    app_bootstrap_on,
    client_bootstrap_on,
    auth_headers_bootstrap_on,
):
    """G-T4-03: compact_operator_comparison grammar and cross-surface contract regression layers."""
    from . import test_area2_task3_closure_gates as _t3
    from . import test_cross_surface_operator_audit_contract as _xs

    await _t3.test_g_t3_01_compact_truth_model_gate(
        app_bootstrap_on,
        client_bootstrap_on,
        auth_headers_bootstrap_on,
    )
    _xs.test_g_conv_08_cross_surface_area2_truth_coherence(client_bootstrap_on, auth_headers_bootstrap_on)


@pytest.mark.asyncio
async def test_g_t4_04_negative_degraded_honesty_gate(minimal_module):
    """G-T4-04: degraded Runtime paths and Improvement missing-adapter honesty."""
    from . import test_runtime_task4_hardening as _hard
    ...
    imp_neg = _load_tests_py_module("improvement/test_improvement_task2a_routing_negative.py")
    imp_neg.test_run_routed_bounded_call_missing_provider_adapter_skips_with_skip_reason()


def test_g_t4_05_drift_resistance_gate():
    """G-T4-05: audit schema and routing-evidence stable key expectations."""
    from . import test_task4_drift_resistance as _drift
...
def test_g_t4_06_validation_command_reality_gate():
    ...
    closure_report = (REPO_ROOT / "docs" / "architecture" / "area2_validation_hardening_closure_report.md").read_text(
        encoding="utf-8"
    )
...
def test_g_t4_07_required_suite_stability_gate():
...
def test_g_t4_08_documentation_and_closure_truth_gate():
    """G-T4-08: required architecture docs reference every G-T4 gate and the command surface."""
    gate_doc = REPO_ROOT / "docs" / "architecture" / "area2_task4_closure_gates.md"
    ...
    doc_paths = (
        ...
        REPO_ROOT / "docs" / "architecture" / "area2_validation_hardening_closure_report.md",
