...
def test_player_shell_projection_target_consumer_and_actor_id_match_player() -> None:
    snap = _snapshot(
        _situation_with_all_dimensions(
            actor_id="annette",
            actor_type=W5ActorType.HUMAN,
    ...
    assert proj.target_consumer is W5ProjectionConsumer.PLAYER_SHELL
    assert proj.actor_id == "annette"
    assert proj.where_summary["current_visible_location"] == "salon"
    assert proj.where_summary["scene_location"]["value"] == "salon"


def test_player_shell_projection_contains_all_dimensions_and_how_is_first_class() -> None:
    snap = _snapshot(
        _situation_with_all_dimensions(
            actor_id="annette",
            actor_type=W5ActorType.HUMAN,
    ...
    proj = build_w5_projection_for_player_shell(snap, player_actor_id="annette")
    assert proj.who_summary["actor_type"] == "human"
    assert proj.where_summary["facts"]["scene_location"] == "salon"
    assert proj.what_summary["facts"]["current_action"] == "listens"
    assert proj.how_summary["facts"]["tone"] == "strained"
    assert proj.why_summary["facts"]["motive"] == "defend_son"
    assert "tone" not in proj.what_summary["facts"]
    assert proj.truth_attribution["why_summary.facts.motive"] == "inferred"


def test_player_shell_projection_accepts_typed_snapshot_and_persisted_dict() -> None:
    typed = _snapshot(
        _situation_with_all_dimensions(
            actor_id="annette",
            actor_type=W5ActorType.HUMAN,
    ...
    proj_typed = build_w5_projection_for_player_shell(typed, player_actor_id="annette")
    proj_dict = build_w5_projection_for_player_shell(typed.to_dict(), player_actor_id="annette")
    assert proj_typed.to_dict() == proj_dict.to_dict()
    assert proj_dict.source_attribution["how_summary.facts.tone"] == "committed_action"
    assert proj_dict.truth_attribution["how_summary.facts.tone"] == "observed"


def test_player_shell_projection_does_not_leak_private_npc_why_without_scope() -> None:
    annette = _situation_with_all_dimensions(
        actor_id="annette",
        actor_type=W5ActorType.HUMAN,
        location="salon",
    ...
    snap = W5Snapshot(
        snapshot_id="w5s_player_no_npc_private",
        story_session_id="sess_proj_test",
        turn_number=TURN,
        created_at=f"w5:turn:{TURN}",
        actors={"annette": annette, "michel": michel},
    )
    proj = build_w5_projection_for_player_shell(snap, player_actor_id="annette")
    assert proj.where_summary["nearby_interactable_actors"] == [
        {"actor_id": "michel", "actor_type": "npc", "scene_location": "salon"}
    ]
    assert "known_actors" not in proj.why_summary
    assert "defend_son" in repr(proj.why_summary)
    assert "michel" not in repr(proj.why_summary)


def test_player_shell_projection_exposes_private_npc_why_when_scope_allows() -> None:
    annette = _situation_with_all_dimensions(
        actor_id="annette",
        actor_type=W5ActorType.HUMAN,
        location="salon",
    ...
    proj = build_w5_projection_for_player_shell(snap, player_actor_id="annette")
    assert proj.why_summary["known_actors"]["michel"]["facts"]["motive"] == "avoid_blame"
    assert (
        proj.truth_attribution["why_summary.known_actors.michel.facts.motive"]
        == "inferred"
    )


def test_player_shell_projection_does_not_leak_other_player_private_facts() -> None:
    annette = _situation_with_all_dimensions(
        actor_id="annette",
        actor_type=W5ActorType.HUMAN,
        location="salon",
    ...
    alain_private = _fact(
        fact_id="w5f_alain_private_player_why",
        actor_id="alain",
        dimension=W5Dimension.WHY,
        key="motive",
        value="conceal_embarrassment",
        source=W5Source.CHARACTER_MIND_RECORD,
        truth=W5TruthLevel.INFERRED,
        visibility=W5VisibilityScope.PRIVATE_TO_ACTOR,
        actor_knowledge_scope=("annette",),
    ...
    snap = W5Snapshot(
        snapshot_id="w5s_player_private_no_leak",
        story_session_id="sess_proj_test",
        turn_number=TURN,
        created_at=f"w5:turn:{TURN}",
        actors={"annette": annette, "alain": alain},
    )
    proj = build_w5_projection_for_player_shell(snap, player_actor_id="annette")
    assert "known_actors" not in proj.why_summary
