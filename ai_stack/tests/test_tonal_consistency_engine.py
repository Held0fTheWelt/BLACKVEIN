...
def test_tonal_consistency_validation_uses_structured_classification() -> None:
    policy = _policy()
    ...
    degraded = validate_tonal_consistency_realization(
        tonal_consistency_target=target,
        structured_output={
    ...
    assert degraded["contract_pass"] is False
    assert "tonal_consistency_required_dimension_missing" in degraded["failure_codes"]
    assert "tonal_consistency_forbidden_marker_detected" in degraded["failure_codes"]
    assert degraded["actual"]["forbidden_marker_hits"][marker_class] >= 1
    for code in degraded["failure_codes"]:
