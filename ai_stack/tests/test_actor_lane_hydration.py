"""Unit tests for narrative-only → actor-lane hydration."""
...
from ai_stack.story_runtime.actor_lane_hydration import (
    apply_structured_hydration,
    hydrate_actor_lanes,
    should_hydrate_actor_lanes,
)
...
def test_hydration_module_avoids_scene_energy_runtime_literal() -> None:
    """Hydration must not reintroduce scene_energy as a production shortcut literal."""
    text = (
        Path(__file__).resolve().parents[1] / "story_runtime" / "actor_lane_hydration.py"
    ).read_text(
        encoding="utf-8"
    )
...
def test_should_hydrate_when_narrative_only() -> None:
    ...
    assert should_hydrate_actor_lanes(
        structured,
...
def test_hydration_skips_narrator_only_local_consequence() -> None:
    ...
    hydrated = apply_structured_hydration(
        {
            "module_id": "god_of_carnage",
    ...
    assert "actor_lane_hydrated" not in hydrated["metadata"]
