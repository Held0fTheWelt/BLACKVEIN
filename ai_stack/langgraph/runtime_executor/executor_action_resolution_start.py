"""Action resolution setup.

Starts player-action resolution with semantic action, environment state, local context, and narrator consequence planning inputs.
"""
SOURCE_LINES = [
    ...
    '                    else {}\n',
    '                )\n',
    '                # Phase 6B-3A — F21 W5-first read.\n',
    '                # The committed actor-location substrate continues to live in\n',
    '                # ``state.get("actor_locations")`` and\n',
    '                # ``environment_state.actor_locations``.  Under default-on W5\n',
    '                # we prefer the typed ``where_summary.derived_actor_locations``\n',
    '                # projection.  Legacy substrate is retained verbatim for:\n',
    '                #   - explicit_opt_out_legacy  (W5_AST_DIRECTOR_PROJECTION_ENABLED=0/false/no/off)\n',
    '                #   - malformed_w5_fallback    (snapshot present but projection raises)\n',
    '                #   - old_payload_legacy       (no w5_latest_snapshot in state)\n',
    '                _pr_c_legacy_actor_locations = (\n',
    '                    state.get("actor_locations")\n',
    '                    if isinstance(state.get("actor_locations"), dict)\n',
    '                    else {}\n',
    '                )\n',
    '                if not _pr_c_legacy_actor_locations:\n',
    '                    if isinstance(_pr_c_env_state.get("actor_locations"), dict):\n',
    '                        _pr_c_legacy_actor_locations = dict(_pr_c_env_state["actor_locations"])\n',
    '                _pr_c_w5_first_resolution = resolve_w5_first_actor_locations(\n',
    '                    legacy_actor_locations=_pr_c_legacy_actor_locations,\n',
    '                    w5_latest_snapshot=(\n',
    ...
    '                    ),\n',
    '                )\n',
    '                _pr_c_actor_locations_raw = _pr_c_w5_first_resolution["actor_locations"]\n',
    '                _pr_c_actor_locations_source = _pr_c_w5_first_resolution["source"]\n',
    '                _pr_c_actor_locations_source_w5_snapshot_id = (\n',
    '                    _pr_c_w5_first_resolution.get("w5_snapshot_id")\n',
    '                )\n',
    '                _pr_c_actor_locations_source_failure_reason = (\n',
    '                    _pr_c_w5_first_resolution.get("failure_reason")\n',
    '                )\n',
