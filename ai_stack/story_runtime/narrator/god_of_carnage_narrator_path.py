...
scripted-with-player-window steps (005+), adding NPC speech directives for
``director_instruction.npc_speak`` beats. World-Engine realization may render
those directives as one narrated prose block with embedded speech spans, while
deterministic ``narrator_perception_only`` rendering stays unchanged.
"""
...
NARRATOR_PATH_CONTRACT = "god_of_carnage_narrator_path.opening.v1"
SCRIPTED_CONTINUATION_CONTRACT = "god_of_carnage_narrator_path.scripted_continuation.v1"
...
def _npc_speak_block(
    *,
    ...
    """Build a pending NPC-speech scene block from an ``npc_speak`` directive.

    The ``text`` field is a placeholder that will be replaced by LLM
    realization in the manager.  We embed the full directive as
    ``source_facts.npc_speak`` so the realization prompt has everything. The
    manager may realize the block as one ``narrator`` /
    ``narrated_actor_speech`` visible card with ``embedded_speech_spans``.
    """
...
def build_goc_scripted_continuation(
    *,
    ...
    - ``scene_blocks``: list of blocks (``narrator`` for perception lines,
      pending NPC-speech blocks with ``requires_llm_realization=True`` for
      npc_speak)
