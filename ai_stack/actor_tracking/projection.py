...
def _coerce_snapshot(snapshot: W5Snapshot | Mapping[str, Any] | None) -> W5Snapshot | None:
    ...
    raise TypeError(
        "W5 projection builder snapshot must be W5Snapshot, "
...
def _player_can_read_fact(
    *,
    ...
    if is_player_actor:
        return fact.visibility is W5VisibilityScope.PRIVATE_TO_ACTOR
    if situation.actor_type is W5ActorType.HUMAN:
        # Another human actor's private/player-private facts do not leak into
        # the local player's shell projection.
        return False
    return (
        fact.visibility is W5VisibilityScope.PRIVATE_TO_ACTOR
        and player_actor_id in set(fact.actor_knowledge_scope)
    )
...
def _player_dimension_summary(
    *,
    ...
    summary: dict[str, Any] = {"actor_id": player_actor_id, "facts": {}}
    if player_situation is not None:
        own_facts = _active_facts(getattr(player_situation, dimension_name))
        seen: set[str] = set()
        for fact in own_facts:
            if fact.key in seen:
                continue
...
def _empty_projection(target_consumer: W5ProjectionConsumer) -> W5Projection:
    ...
    if target_consumer is W5ProjectionConsumer.PLAYER_SHELL:
        return W5Projection(
            schema_version=W5_PROJECTION_SCHEMA_VERSION,
            target_consumer=W5ProjectionConsumer.PLAYER_SHELL,
            actor_id=None,
            who_summary={},
            where_summary={
                "facts": {},
                "visible_actors": [],
                "nearby_interactable_actors": [],
                "w5_snapshot_id": None,
            },
            what_summary={"facts": {}},
            how_summary={"facts": {}},
            why_summary={"facts": {}},
            source_attribution={
                "where_summary.w5_snapshot_id": "w5_snapshot",
            },
            truth_attribution={
                "where_summary.w5_snapshot_id": "observed",
            },
        )
    return W5Projection(
        schema_version=W5_PROJECTION_SCHEMA_VERSION,
...
def _select_player_actor_id(
    snapshot: W5Snapshot,
    *,
    player_actor_id: str | None,
) -> str | None:
    requested = str(player_actor_id or "").strip()
    if requested and requested in snapshot.actors:
        return requested
    if requested:
        by_lower = {aid.lower(): aid for aid in snapshot.actors}
...
def _player_visible_scene_location(
    *,
    ...
    active = _active_facts(situation.where)
    fact = _pick_strongest(active, "scene_location")
...
def build_w5_projection_for_player_shell(
    ...
    *,
    ...
    for actor_id in sorted(typed_snapshot.actors.keys()):
        situation = typed_snapshot.actors[actor_id]
        location_fact = _player_visible_scene_location(
            situation=situation,
            player_actor_id=chosen_actor_id,
            is_player_actor=(actor_id == chosen_actor_id),
        )
        if location_fact is None:
            continue
        visible_actors.append(
            {
                "actor_id": actor_id,
                "actor_type": situation.actor_type.value,
                "scene_location": str(location_fact.value).strip(),
            }
        )
        _record_attribution(
            source_attribution=source_attribution,
    ...
    where_summary["player_actor_situation"] = {
        ...
        "how": dict(how_summary.get("facts") or {})
