...
def test_create_run_and_ticket(tmp_path: Path):
    ...
    response = client.post(
        "/api/runs",
        json={"template_id": "god_of_carnage_solo", "account_id": "acct:hollywood", "display_name": "Hollywood"},
    )
    ...
    ticket_response = client.post(
        "/api/tickets",
        json={"run_id": run_id, "account_id": "acct:hollywood", "display_name": "Hollywood"},
    )
...
def test_websocket_move_flow(tmp_path: Path):
    ...
    with client.websocket_connect(f"/ws?ticket={ticket}") as websocket:
        ...
        assert first_payload["data"]["current_room"]["id"] == "hallway"
