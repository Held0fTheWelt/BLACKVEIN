...
|-------|-------------------------------------|-------------------------------------|
| **As of (date & time)** | — | **2026-05-20 (UTC, post DS-001–005 closure)** |
...
| Extra check runtime | — | **`ds005_runtime_import_check.py`** exit **0** (2026-05-20); grep `TYPE_CHECKING` / `avoid circular` / `circular dependency` under `backend/app/runtime`: **0** hits |
| **Open hotspots** | — | **DS-001–005 closed** on current tree (re-run **`spaghetti-check --with-metrics`** for fresh **C1..C7** / **M7**). **Remaining advisory:** long modules e.g. `forum_service`, `area2_operator_truth`; **C3** Trigger v2 was high at last full scan — refresh metrics before next DS row. |
...
Each **open** row: **ID**, **pattern** (lead with **C1..C7** from [spaghetti-setup.md](../spaghetti-setup.md) § *Per-category trigger bars*, e.g. **`C3 ·`** …), **location**, **hint / measurement idea**, **direction**, **collision hint** (what is risky in parallel).
...
**Batch closed 2026-05-20:** **DS-001** (C1) → **DS-002** (C4) → **DS-004** (C5) → **DS-003** (C6) → **DS-005** (C7). Per-DS outcomes, gates, and artefact paths: **[despaghettification_completed_log.md](despaghettification_completed_log.md)** § *Completed waves*. Workstream evidence: `WORKSTREAM_BACKEND_RUNTIME_SERVICES_STATE.md`, `WORKSTREAM_AI_STACK_STATE.md`.

**New rows:** next **DS-006**+ when check fills the open table; **pattern** starts with **C1..C7** per [spaghetti-check-task.md](../spaghetti-check-task.md) §2. On closure, append [despaghettification_completed_log.md](despaghettification_completed_log.md) and remove from *Open* above.
...
Executed **2026-05-20** in order **1→2→3→4→5** (**DS-001** → **DS-002** → **DS-004** → **DS-003** → **DS-005**). Detail and gates: [despaghettification_completed_log.md](despaghettification_completed_log.md). Historical flow (reference only):

```mermaid
flowchart TB
  P1["1 · DS-001 · runtime C1 seams · CLOSED"]
  P2["2 · DS-002 · C4 services/runtime · CLOSED"]
  P3["3 · DS-004 · C5 constants · CLOSED"]
  P4["4 · DS-003 · C6 ai_stack dedupe · CLOSED"]
  P5["5 · DS-005 · C7 control flow · CLOSED"]
  P1 --> P2 --> P3 --> P4 --> P5
```

**Fill in:** one phase row per **open** **DS-*** when check repopulates the backlog. Order by **risk** (runtime/import before large orchestrators). **Mermaid:** omit while the open table is only `—`. Closed phases stay in the completed log, not in the open table above.
