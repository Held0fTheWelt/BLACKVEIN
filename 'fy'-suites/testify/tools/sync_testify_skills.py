#!/usr/bin/env python3
"""Copy Testify hub skills into ``.cursor/skills/`` for Cursor project skill discovery.

Canonical sources: ``'fy'-suites/testify/superpowers/<skill-name>/SKILL.md``
Targets:          .cursor/skills/<skill-name>/SKILL.md

Run from repository root after editing any skill under ``'fy'-suites/testify/superpowers/``:

  python "./'fy'-suites/testify/tools/sync_testify_skills.py"
...
TESTIFY_ROOT = Path(__file__).resolve().parents[1]
REPO_ROOT = resolve_project_root(start=Path(__file__), marker_text=None)
SRC_ROOT = TESTIFY_ROOT / "superpowers"
DST_ROOT = REPO_ROOT / ".cursor" / "skills"


def sync(*, check_only: bool) -> int:
    """Copy Testify router skills to ``.cursor/skills`` or verify they match."""
    ...
    if check_only:
        if mismatches:
            print("Testify skill sync drift:\n" + "\n".join(mismatches), file=sys.stderr)
            return 1
        print("OK: .cursor/skills matches 'fy'-suites/testify/superpowers")
        return 0
    return 0
