...
A — Explicit contract candidates (high confidence when markers or known hub paths match).
B — Structural boundary candidates (medium): workflows, public integration surfaces.
C — Referencing / audience artifacts (usually projections, not anchors).
D — Out of scope by default (not scanned here).
E — Confidence recorded per row; ``discovery_reason`` explains classification.
"""
...
def discover_contracts_and_projections(
    ...
    *,
    ...
    """Return contracts, projections, and high-confidence relation edges (phase-1 ceiling)."""
    repo = repo.resolve()
...
def projection_backref_ok(md_path: Path, *, normative_fragment: str = "normative-contracts-index") -> tuple[bool, str]:
    """Heuristic C-tier: audience markdown should link back to normative index or carry machine marker."""
