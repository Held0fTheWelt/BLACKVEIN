...
from dockerify.tools.docker_audit import write_audit_bundle
...
def _print_help() -> None:
    print(
        'Dockerify hub CLI\n\n'
        'Commands:\n'
        '  audit      Audit Docker / compose governance surfaces.\n'
...
def _audit(args: argparse.Namespace) -> int:
    root = repo_root()
    json_rel = args.out or "'fy'-suites/dockerify/reports/dockerify_audit.json"
    md_rel = args.md_out or "'fy'-suites/dockerify/reports/dockerify_audit_report.md"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not argv or argv[0] in ('-h', '--help', 'help'):
        _print_help()
        return 0
    parser = argparse.ArgumentParser(description='Dockerify repository docker governance CLI')
