#!/usr/bin/env python3
"""Copy Documentify hub skills into ``.cursor/skills/`` for Cursor project skill discovery.

Canonical sources: ``'fy'-suites/documentify/superpowers/<skill-name>/SKILL.md``
Targets:          .cursor/skills/<skill-name>/SKILL.md

Run from repository root after editing any skill under ``'fy'-suites/documentify/superpowers/``:

  python "./'fy'-suites/documentify/tools/sync_documentify_skills.py"
...
DOCUMENTIFY_ROOT = Path(__file__).resolve().parents[1]
REPO_ROOT = resolve_project_root(start=Path(__file__), marker_text=None)
SRC_ROOT = DOCUMENTIFY_ROOT / "superpowers"
DST_ROOT = REPO_ROOT / ".cursor" / "skills"


def sync(*, check_only: bool) -> int:
    """Copy Documentify router skills to ``.cursor/skills`` or verify they match."""
    ...
    if check_only:
        if mismatches:
            print("Documentify skill sync drift:\n" + "\n".join(mismatches), file=sys.stderr)
            return 1
        print("OK: .cursor/skills matches 'fy'-suites/documentify/superpowers")
        return 0
    return 0
